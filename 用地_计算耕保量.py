...
def main(
    ...
    输入要素名称="DIST_用地规划图",
    有扣除地类系数的要素名称="CZ_三调筛选_有扣除地类用地",
    输出要素名称=r"DIST_用地规划图_计算耕保量",
):  # 用地_计算耕保量
    ...
    with bxarcpy.类.环境.环境管理器(临时工作空间=工作空间, 工作空间=工作空间):
        ...
        用地规划图 = bxarcpy.类.要素类.要素读取_通过名称(输入要素名称)
        仅有耕地的要素 = 用地规划图.要素创建_通过筛选("地类编号 LIKE '01%'")
        ...
        # 输出要素 = bxarcpy.分析.相交(输入要素列表=[有扣除地类系数的要素名称, 输出要素])
        ...
        # Process: 联合
        用地规划图.字段删除(["扣除地类系数", "耕地保有量"])
        合并扣除系数后要素 = 用地规划图.要素创建_通过联合([带有扣除系数的耕地要素.名称])
        ...
        耕地保有量汇总 = 0
        with bxarcpy.类.游标类_用于更新(合并扣除系数后要素.名称, ["地类编号", "扣除地类系数", "SHAPE@AREA", "耕地保有量"]) as 游标:
            for x in 游标:
                if 字.匹配正则(x.数据_列表格式[0], "^01"):
                    耕地保有量 = (1 - x.数据_列表格式[1]) * x.数据_列表格式[2]
                    耕地保有量汇总 += 耕地保有量
                    x.数据_列表格式[3] = 耕地保有量
                    x.行更新_列表格式()
        ...
        合并扣除系数后要素.要素创建_通过复制并重命名重名要素(输出要素名称)
        # bxarcpy.数据管理.要素复制(输入要素=输出要素, 输出要素=输出要素名称)

...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        # 用地创建_通过计算所属区域("DIST_用地规划图", 区域要素名称="JX_街坊范围线", 字段映射列表=[["所属街坊", "区域编号"]], 计算方式="分割输入要素" , 输出要素名称="DIST_用地规划图1")
        # 用地创建_通过计算所属区域("DIST_用地规划图1", 区域要素名称="JX_街区范围线", 字段映射列表=[["所属街区", "区域编号"]], 计算方式="分割输入要素", 输出要素名称="DIST_用地规划图2")
        用地创建_通过计算所属区域("DIST_用地规划图", 区域要素名称="KZX_城镇开发边界", 字段映射列表=[["所属三线", "控制线名称"]], 计算方式="按输入要素内点", 输出要素名称="DIST_用地规划图1")

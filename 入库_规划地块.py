...
def 入库_用地(地块要素名称="DIST_用地规划图", 单元名称="临江单元", 批复时间="", 批复文号="", 地块编号字段名称="地块编号", 地类编号字段名称="地类编号", 性质名称字段名称="性质名称", 地块性质别称字段名称="地块性质别称", 兼容比例字段名称="兼容比例"):
    ...
    with bxarcpy.游标类.游标创建_通过名称("更新", 地块要素.名称, 需操作的字段名称列表) as 游标:
        for x in 游标:
            ...
            if x["开发动态"] in ["现状已实施"]:
                x["GHDT"] = "保留"
            elif x["开发动态"] in ["", " ", None] and x["地类编号"][0:2] not in ["01", "02", "03", "04", "05", "06", "17", "14"] and x["地类编号"][0:4] not in ["1207"]:
                x["GHDT"] = "新建"

            if x["兼容比例"] not in ["", " ", None]:
                x["JRBL"] = x["兼容比例"].replace("/", ":")
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        入库_用地(地块要素名称="DIST_用地规划图", 单元名称="临江单元", 批复时间="2023/12/18", 批复文号="杭政函〔2023〕109号")

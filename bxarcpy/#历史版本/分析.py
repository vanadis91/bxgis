...
def 填充空隙(输入要素名称, 填充范围要素名称, 填充地类编号表达式='"00"', 输出要素名称="in_memory\\AA_填充空隙"):
    # 常用_填充空隙后
    # To allow overwriting outputs change overwriteOutput option to True.
    if 输出要素名称 == "in_memory\\AA_填充空隙":
        输出要素名称 = 输出要素名称 + "_" + time.strftime("%Y%m%d%H%M%S", time.localtime())

    # Process: 复制要素 (复制要素) (management)
    填充范围要素 = bxarcpy.类.要素类.要素读取_通过名称(填充范围要素名称)
    填充范围1 = 填充范围要素.要素创建_通过复制()
    # 填充范围1 = bxarcpy.数据管理.要素复制(输入要素=填充范围要素名称)
    ...
    填充范围1.要素创建_通过更新并合并字段(输入要素名称, 输出要素名称)
    # 输出要素名称 = 更新并合并字段(输入要素=填充范围1, 更新要素=输入要素名称, 输出要素=输出要素名称)
    return 输出要素名称

...
def 设施创建_通过计算所属区域(输入要素名称, 区域要素名称="JX_工业片区范围线", 字段映射列表=[["所属工业片区", "区域编号"]], 输出要素名称="in_memory\\AA_计算所属区域"):
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        设施创建_通过计算所属区域("SS_配套设施_移动至正确位置_清理范围外设施", 区域要素名称="DIST_用地规划图", 字段映射列表=[["设施所在地块编号", "地块编号"]], 输出要素名称="SS_配套设施_移动至正确位置_清理范围外设施_所属区域")

import bxarcpy
from 设施_设施更新_020所属区域 import 设施创建_通过计算所属区域


def 设施创建_通过计算远期预留(输入要素名称, 区域要素名称="KZX_城镇开发边界", 字段映射列表=[["设施所在用途分区", "控制线名称"]], 输出要素名称="in_memory\\AA_计算远期预留"):
    if 输出要素名称 == "in_memory\\AA_计算远期预留":
        输出要素名称 = 输出要素名称 + "_" + bxarcpy.工具集.生成短GUID()

    设施要素 = bxarcpy.要素类.要素读取_通过名称(输入要素名称).要素创建_通过复制()
    设施要素 = 设施创建_通过计算所属区域(设施要素.名称, 区域要素名称, 字段映射列表)
    设施要素.字段添加("远期预留")
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        设施创建_通过计算远期预留("SS_配套设施_移动至正确位置_清理范围外设施_所属区域", 区域要素名称="CZ_控制线_三线_城镇开发边界", 字段映射列表=[["设施所在用途分区", "GHFQMC"]], 输出要素名称="SS_配套设施_移动至正确位置_清理范围外设施_所属区域_远期预留")

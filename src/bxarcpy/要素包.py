...
class 要素类:
    ...
    @staticmethod
    def 要素创建_通过复制(要素路径, 输出要素路径="内存临时"):
        日志类.临时关闭日志()
        from bxpy.路径包 import 路径类
        from bxpy.基本对象包 import 字类

        日志类.输出调试(f"要素路径为：{要素路径}")
        输出要素路径 = _输出路径生成_当采用内存临时时([要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    @staticmethod
    def 要素创建_通过复制并重命名重名要素(输入要素路径, 输出要素路径="内存临时", 重名时原要素被重命名后后缀=""):
        from bxarcpy.数据库包 import 数据库类
        from bxpy.路径包 import 路径类
        from bxarcpy.环境包 import 环境类, 输入输出类

        输出要素路径 = _输出路径生成_当采用内存临时时([输入要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    def 要素创建_通过合并(输入要素路径列表=[], 输出要素路径="内存临时"):
        输出要素路径 = _输出路径生成_当采用内存临时时(输入要素路径列表) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    def 要素创建_通过裁剪(输入要素路径, 裁剪要素路径, 输出要素路径="内存临时"):
        输出要素路径 = _输出路径生成_当采用内存临时时([输入要素路径, 裁剪要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    def 要素创建_通过擦除_原始(输入要素路径, 擦除要素路径, 输出要素路径="内存临时"):
        ...
        输出要素路径 = _输出路径生成_当采用内存临时时([输入要素路径, 擦除要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    def 要素创建_通过分割(输入要素路径, 分割要素路径, 输出要素路径="内存临时"):
        输出要素路径 = _输出路径生成_当采用内存临时时([输入要素路径, 分割要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    def 要素创建_通过联合并赋值字段(输入要素路径, 联合要素路径="JX_街区范围线", 字段映射列表=[["所属街区", "街区编号"]], 是否多部件转单部件=True, 是否去除输入有联合无的部分=False, 是否去除输入无联合有的部分=False, 是否去除输入有联合有的部分=False, 是否检查两者差异=True, 差异是否输出到要素类=False, 差异是否输出到CAD=False, 要素被分割时提示信息中包括的字段=[], 是否删除字段的既有值=True, 输出字段设置: Literal["所有", "除FID外所有字段", "仅FID字段", "仅第一个要素字段"] = "仅第一个要素字段", 输出要素路径="内存临时"):
        ...
        if 是否检查两者差异:
            ...
            日志类.输出调试(f"联合后要素字段列表为：{联合后要素字段列表}")
            元素FID列表 = []
            元素属性列表 = []
            分割要素已插入列表 = []
            from bxarcpy.游标包 import 游标类
        ...
        # 开始操作字段
        日志类.输出调试(f"联合后要素的字段列表：{要素类.字段名称列表获取(联合后要素路径)}", 内容长度=20000)
    ...
    def 要素创建_通过更新(输入要素路径, 更新要素路径, 输出要素路径="内存临时"):
        输出要素路径 = _输出路径生成_当采用内存临时时([输入要素路径, 更新要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    @staticmethod
    def 要素创建_通过筛选(输入要素路径, SQL语句="", 输出要素路径="内存临时"):
        输出要素路径 = _输出路径生成_当采用内存临时时([输入要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    def 要素创建_通过填充空隙(输入要素路径, 填充范围要素路径, 填充地类编号表达式='"00"', 输出要素路径="内存临时"):
        ...
        输出要素路径 = _输出路径生成_当采用内存临时时([输入要素路径, 填充范围要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    def 要素创建_通过投影转换(输入要素路径, 输出坐标系='PROJCS["CGCS2000_3_Degree_GK_CM_120E",GEOGCS["GCS_China_Geodetic_Coordinate_System_2000",DATUM["D_China_2000",SPHEROID["CGCS2000",6378137.0,298.257222101]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Gauss_Kruger"],PARAMETER["False_Easting",500000.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",120.0],PARAMETER["Scale_Factor",1.0],PARAMETER["Latitude_Of_Origin",0.0],UNIT["Meter",1.0]]', 输出要素路径="内存临时"):
        ...
        复制后要素路径 = 要素类.要素创建_通过复制(输入要素路径)
        arcpy.management.Project(in_dataset=复制后要素路径, out_dataset=输出要素路径, out_coor_system=输出坐标系, transform_method=None, in_coor_system=None, preserve_shape="NO_PRESERVE_SHAPE", max_deviation=None, vertical="NO_VERTICAL")  # type: ignore
        return 输出要素路径
    ...
    @staticmethod
    def 字段添加_添加面积字段(输入要素路径):
        字段名称列表 = 要素类.字段名称列表获取(输入要素路径)
        if "Shape_Area" not in 字段名称列表:
            要素类.字段添加(输入要素路径, "Shape_Area", "双精度")
            from bxarcpy.游标包 import 游标类
    ...
    def 拓扑检查重叠_通过要素名称列表(输入要素路径列表, 是否导出到CAD=None):
        ...
        for 输入要素路径x in 进度条:
            ...
            日志类.输出调试(f"开始对以下要素类进行拓扑检查：{输入要素路径x}")

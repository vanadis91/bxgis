...
class 要素类:
    ...
    def 要素创建_通过联合并赋值字段(self, 区域要素名称="JX_街区范围线", 字段映射列表=[["所属街区", "街区编号"]], 是否检查两者差异=True, 差异输出到CAD时目录=False, 输出要素名称="in_memory\\AA_联合并赋值字段"):
        ...
        if 是否检查两者差异:
            ...
            if 应填色但是没填的区域要素.几何数量 > 0:
                ...
                应填色但是没填的区域要素.要素创建_通过复制并重命名重名要素(f"AA_{输入要素原始无路径名称}与{区域要素原始无路径名称}之间应填未填区域")
                if 差异输出到CAD时目录:
                    应填色但是没填的区域要素.导出到CAD(str(差异输出到CAD时目录) + rf"\AA_{输入要素原始无路径名称}与{区域要素原始无路径名称}之间应填未填区域.dwg")

            if 不应填色但是填色的区域要素.几何数量 > 0:
                环境.输出消息(f"{输入要素原始无路径名称}与{区域要素原始无路径名称}之间存在不应填但被填区域")
                不应填色但是填色的区域要素.要素创建_通过复制并重命名重名要素(f"AA_{输入要素原始无路径名称}与{区域要素原始无路径名称}之间不应填但被填区域")
                if 差异输出到CAD时目录:
                    不应填色但是填色的区域要素.导出到CAD(str(差异输出到CAD时目录) + rf"\AA_{输入要素原始无路径名称}与{区域要素原始无路径名称}之间不应填但被填区域.dwg")

            if 被范围线分割的要素.几何数量 > 0:
                环境.输出消息(f"{输入要素原始无路径名称}被{区域要素原始无路径名称}联合后有些要素被分割")
                被范围线分割的要素.要素创建_通过复制并重命名重名要素(f"AA_{输入要素原始无路径名称}被{区域要素原始无路径名称}联合后被分割图形")
                if 差异输出到CAD时目录:
                    被范围线分割的要素.导出到CAD(str(差异输出到CAD时目录) + rf"\AA_{输入要素原始无路径名称}被{区域要素原始无路径名称}联合后被分割图形.dwg")
                    ...
                    for x in 面要素游标对象:
                        ...
                        for y in 点要素游标对象:
                            if int(x["_ID"]) == y["FID_" + 目标要素.名称_无路径]:
                                ...
                                break
                        if findFlag == False:
                            日志.输出信息(f'未找到ID为{x["_ID"]}的对象所对应的点')
    ...
    def 拓扑检查重叠_通过要素名称列表(输入要素名称列表):
        ...
        for x in 输入要素名称列表:
            ...
            要素数据集中的要素 = 输入要素.导出到要素(要素数据集.名称 + "\\" + 输入要素.名称_无路径 + "_1")

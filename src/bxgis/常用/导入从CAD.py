...
def 导入从CAD(输入CAD数据集中的要素类=r"C:\Users\beixiao\Desktop\01.dwg\控规地块", 是否拓扑检查=False, 是否范围检查=True, 是否转曲=True, 输出要素名称=r"YD_CAD色块"):
    ...
    输入要素 = bxarcpy.要素类.要素读取_通过名称(输入CAD数据集中的要素类)
    ...
    if 输入要素.名称_无路径 == "控规地块":
        复制后要素 = 复制后要素.字段添加(字段名称=配置.地块要素字段映射.地类编号字段名称).字段计算(配置.地块要素字段映射.地类编号字段名称, "!Layer!.split('#')[0].split('-')[1].replace('／','/')")
    ...
    if 是否范围检查:
        复制后要素.拓扑检查范围(配置.项目信息.JX_规划范围线要素名称)

    输出要素 = 复制后要素.要素创建_通过复制并重命名重名要素(输出要素名称)
    return 输出要素
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        导入从CAD(输入CAD数据集中的要素类=r"C:\Users\beixiao\Desktop\01.dwg\控规地块", 是否拓扑检查=True, 是否范围检查=True, 是否转曲=True, 输出要素名称=r"YD_CAD色块222")

...
def 导出到CAD(输入要素名称="DIST_用地规划图", 规划范围线要素名称: Union[str, None] = "JX_规划范围线", 需融合字段名称=None, 需融合字段中需融合的值的列表=None, 切分阈值=None, 是否去孔=True, CAD中图层采用的字段的名称=None, 输出CAD路径=r"C:\Users\beixiao\Desktop\01.dwg"):
    ...
    if 需融合字段名称:
        if 需融合字段中需融合的值的列表:
            for 需融合字段中需融合的值x in 需融合字段中需融合的值的列表:
                ...
                融合后要素 = 选择集.要素创建_通过融合(融合字段列表=[需融合字段名称])
    ...
    if 切分阈值:
        ...
        with bxarcpy.游标类.游标创建_通过名称("更新", 融合后要素.名称, ["_形状", "是否切分", "_ID", "地类编号"]) as 游标:
            for 需融合字段中需融合的值x in 游标:
                图形 = 需融合字段中需融合的值x["_形状"]
                # print(行.数据_列表格式[0])
                # print(type(行.数据_列表格式[0]))
                if bxarcpy.几何对象类.孔洞数量获取(图形) >= 切分阈值["孔洞数量"] and bxarcpy.几何对象类.折点数量获取(图形) >= 切分阈值["折点数量"] and bxarcpy.几何对象类.面积获取(图形) >= 切分阈值["面积"] and 需融合字段中需融合的值x["地类编号"] in 切分阈值["地类编号列表"]:
                    需融合字段中需融合的值x["是否切分"] = "1"
                    游标.行更新(需融合字段中需融合的值x)
                    日志类.输出信息(f"ID：{str(需融合字段中需融合的值x['_ID'])}，孔洞数量：{str(bxarcpy.几何对象类.孔洞数量获取(图形))}, 折点数量：{str(bxarcpy.几何对象类.折点数量获取(图形))}，面积：{str(bxarcpy.几何对象类.面积获取(图形))}，地类：{需融合字段中需融合的值x['地类编号']}")
    ...
    if 是否去孔:
        ...
        with bxarcpy.游标类.游标创建_通过名称("插入", DIST_用地规划图线.名称, [*线要素字段名称列表, "_形状"]) as 游标_线:
            with bxarcpy.游标类.游标创建_通过名称("查询", 切分后要素.名称, [*线要素字段名称列表, "_形状"]) as 游标_面:
                ...
                for 需融合字段中需融合的值x in 进度条类.进度条创建(游标_面, 总进度=切分后要素.几何数量):
                    partList = []
                    for part in bxarcpy.几何对象类.点表获取(需融合字段中需融合的值x["_形状"]):
                        ptList = []
                        shapeList = []
                        for pt in part:
                            if pt is not None:
                    ...
                    需融合字段中需融合的值x["_形状"] = bxarcpy.线.线创建(array)._内嵌对象
                    游标_线.行插入(需融合字段中需融合的值x)
        去孔后要素 = DIST_用地规划图线
    else:
        去孔后要素 = 切分后要素
    if CAD中图层采用的字段的名称:
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        ...
        导出到CAD(输入要素名称="XG_GHDK", 规划范围线要素名称=None, 需融合字段中需融合的值的列表=["1207", "1207v"], 切分阈值=None, 是否去孔=False, CAD中图层采用的字段的名称="dldm", 输出CAD路径=r"C:\Users\beixiao\Desktop\01.dwg")

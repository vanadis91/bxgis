...
def 要素创建_通过更新_根据面(输入要素路径, 区域要素名称="JX_街区范围线", 字段映射列表=[["所属街区", "街区编号"]], 计算方式: Literal["分割输入要素", "内点在区域要素内", "大部分在区域要素内"] = "分割输入要素", 输出要素路径="内存临时"):
    ...
    if 计算方式 == "分割输入要素":
        赋值后要素 = 要素类.要素创建_通过联合并赋值字段(输入要素路径_复制后, 区域要素路径_复制后, 字段映射列表, 要素被分割时提示信息中包括的字段=["_ID", 基本信息.地块要素字段映射.地类编号字段名称])
...
def mian_temp():
    ...
    日志类.输出调试(f"字段映射列表：{字段映射列表}", 内容长度=100000)

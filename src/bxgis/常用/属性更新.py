...
def 常用_属性更新(输入要素路径, 区域要素名称="JX_街区范围线", 字段映射列表=[["所属街区", "街区编号"]], 计算方式: Literal["分割输入要素", "内点在区域要素内", "大部分在区域要素内"] = "分割输入要素", 输出要素路径="内存临时"):
    输出要素路径 = 工具包.临时路径生成(["更新属性_根据面"]) if 输出要素路径 == "内存临时" else 输出要素路径

    输入要素路径_复制后 = 要素类.要素创建_通过复制(输入要素路径)
    区域要素路径_复制后 = 要素类.要素创建_通过复制(区域要素名称)

    区域要素字段列表 = 要素类.字段列表获取(区域要素路径_复制后)

    需要添加的字段列表 = [x[0] for x in 字段映射列表]
    要素类.字段删除(输入要素路径_复制后, 需要添加的字段列表)
    # for i, x in enumerate(需要添加的字段列表):
    #     区域要素中字段名称 = 字段映射列表[i][1]
    #     对应的字段对象 = [y for y in 区域要素字段列表 if 字段类.属性获取_名称(y) == 区域要素中字段名称][0]
    #     要素类.字段添加(输入要素路径_复制后, 字段名称=x, 字段类型=字段类.属性获取_类型(对应的字段对象), 字段长度=字段类.属性获取_长度(对应的字段对象))  # type: ignore
...
def 要素创建_通过更新_根据点(输入要素路径, 点要素名称="AA_test", 字段映射列表=[["所属街区", "街区编号"]], 输出要素路径="内存临时"):
    输出要素路径 = 工具包.临时路径生成(["更新属性_根据点"]) if 输出要素路径 == "内存临时" else 输出要素路径
...
def 要素创建_通过更新_根据点2(输入要素路径, 点要素名称="AA_test", 输出要素路径="内存临时"):
    输出要素路径 = 工具包.临时路径生成(["更新属性_根据点"]) if 输出要素路径 == "内存临时" else 输出要素路径

    输入要素 = 要素类.要素创建_通过复制(输入要素路径)
    点要素 = 要素类.要素创建_通过复制(点要素名称)

    点要素的字段列表 = 要素类.字段名称列表获取(点要素, 含系统字段=False)
    输入要素的字段列表 = 要素类.字段名称列表获取(输入要素, 含系统字段=False)

    for x in 点要素的字段列表:
        if x in 输入要素的字段列表:
            要素类.字段删除(输入要素, [x])
            输入要素的字段列表.remove(x)
    print(f"以下字段没有被更新：{输入要素的字段列表}")

    赋值后要素 = 要素类.要素创建_通过空间连接(输入要素, 点要素, "包含连接要素")
...
if __name__ == "__main__":
    ...
    with 环境管理器类.环境管理器类创建(工作空间):
        用地要素 = 要素创建_通过更新_根据点2("JX_工业片区范围线2", "CZ_工业片区范围线_转点", "JX_工业片区范围线3")
        # mian_temp()

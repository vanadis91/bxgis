...
def 区域更新(
    ...
    with 游标类.游标创建("查询", 区域和耕地相交后, [区域要素中编号字段名称, "净耕地面积"]) as 游标:
        from bxpy.基本对象包 import 字类, 字典类, 浮类

        for x in 游标类.属性获取_数据_字典形式(游标, [区域要素中编号字段名称, "净耕地面积"]):
    ...
    with 游标类.游标创建("查询", 区域和永久基本农田相交后, [区域要素中编号字段名称, "净基本农田面积"]) as 游标:
        from bxpy.基本对象包 import 字类, 字典类, 浮类

        for x in 游标类.属性获取_数据_字典形式(游标, [区域要素中编号字段名称, "净基本农田面积"]):
    ...
    if 生态保护红线要素名称:
        ...
        with 游标类.游标创建("查询", 区域和生态保护红线相交后, [区域要素中编号字段名称, "_面积"]) as 游标:
            from bxpy.基本对象包 import 字类, 字典类, 浮类
    ...
    with 游标类.游标创建("查询", 区域和村庄建设边界相交后, [区域要素中编号字段名称, "_面积"]) as 游标:
        from bxpy.基本对象包 import 字类, 字典类, 浮类

        for x in 游标类.属性获取_数据_字典形式(游标, [区域要素中编号字段名称, "_面积"]):
            字典类.默认值设置(统计数据, [x[区域要素中编号字段名称], "总村庄建设边界用地面积"], 0)
            日志生成器.输出并暂停(f"_面积{x['_面积']}")
            统计数据[x[区域要素中编号字段名称]]["总村庄建设边界用地面积"] += 浮类.转换_到浮(x["_面积"])
            日志生成器.输出并暂停(f"{统计数据[x[区域要素中编号字段名称]]['总村庄建设边界用地面积']}")
        ...
        for x in 游标类.属性获取_数据_字典形式(游标, 需操作的字段名称列表):
            if x[区域要素中编号字段名称] in 统计数据:
                ...
                for 设施大类名称 in ["配套设施汇总", "交通设施汇总", "市政设施汇总", "其他设施汇总"]:
                    ...
                    if 是否计算设施相关内容:
                        x[设施大类名称] = 设施汇总字符串
                    elif not 是否计算设施相关内容 and x[设施大类名称] is None:
                        x[设施大类名称] = ""

                    if len(设施汇总字符串) > 765:
                        x[设施大类名称 + "1"] = x[设施大类名称][0:255]
                        x[设施大类名称 + "2"] = x[设施大类名称][255:510]
                        x[设施大类名称 + "3"] = x[设施大类名称][510:765]
                        from bxarcpy.环境包 import 输入输出类
                    ...
                    if len(设施汇总字符串) > 510:
                        x[设施大类名称 + "1"] = x[设施大类名称][0:255]
                        x[设施大类名称 + "2"] = x[设施大类名称][255:510]
                        x[设施大类名称 + "3"] = x[设施大类名称][510:]
                    elif len(设施汇总字符串) > 255:
                        x[设施大类名称 + "1"] = x[设施大类名称][0:255]
                        x[设施大类名称 + "2"] = x[设施大类名称][255:]
                    else:
                        x[设施大类名称 + "1"] = x[设施大类名称]
            游标类.行更新_字典形式(游标, x)

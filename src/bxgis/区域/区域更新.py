...
def 区域更新(
        ...
        for x in 游标类.属性获取_数据_字典形式(游标_设施, 需操作的字段名称列表):
            if x[区域要素中编号字段名称] in 统计数据:
                ...
                for 设施大类名称 in ["配套设施汇总", "交通设施汇总", "市政设施汇总", "其他设施汇总"]:
                    ...
                    if 是否统计设施:
                        x[设施大类名称] = 设施汇总字符串
                    elif not 是否统计设施 and x[设施大类名称] is None:
                        x[设施大类名称] = ""
    ...
    字段名称列表 = 要素类.字段名称列表获取(区域要素)
    字段名称列表 = [[x, x] for x in 字段名称列表]
    属性对比(区域要素临时, 区域要素, [基本信息.区域要素字段映射.区域编号字段名称, 基本信息.区域要素字段映射.区域编号字段名称], 对比字段名称列表=字段名称列表)

...
def 区域更新(区域要素名称="JX_街坊范围线", 用地要素名称="DIST_用地规划图", 用地要素中所属区域字段名称="所属街区", 区域要素中编号字段名称="区域编号", 永久基本农田要素名称="KZX_基本农田", 生态保护红线要素名称="KZX_生态保护红线", 村庄建设边界要素名称="KZX_村庄建设边界", 设施要素名称="SS_配套设施", 设施要素中所属区域字段名称="所属街区", 输出要素名称="内存临时"):
    ...
    地块要素 = 计算所属区域.计算所属区域(地块要素, 区域要素, [[用地要素中所属区域字段名称, 区域要素中编号字段名称]], "分割输入要素")
    设施要素 = 计算所属区域.计算所属区域(设施要素, 区域要素, [[设施要素中所属区域字段名称, 区域要素中编号字段名称]])
    ...
    with 游标类.游标创建("查询", 区域和永久基本农田相交后, [区域要素中编号字段名称, "Shape_Area"]) as 游标:
        from bxpy.基本对象包 import 字类, 字典类, 浮类

        for x in 游标类.游标迭代器转换_字典形式(游标, [区域要素中编号字段名称, "Shape_Area"]):
            字典类.默认值设置(统计数据, [x[区域要素中编号字段名称], "总永久基本农田用地面积"], 0)
            统计数据[x[区域要素中编号字段名称]]["总永久基本农田用地面积"] += 浮类.转换_到浮(x["Shape_Area"])
    ...
    with 游标类.游标创建("更新", 区域要素, 需操作的字段名称列表) as 游标:
        ...
        for x in 游标类.游标迭代器转换_字典形式(游标, 需操作的字段名称列表):
            if x[区域要素中编号字段名称] in 统计数据:
                ...
                x["总永久基本农田用地面积"] = 字类.转换_到字(统计数据[x[区域要素中编号字段名称]]["总永久基本农田用地面积"])
                x["总生态保护红线用地面积"] = 字类.转换_到字(统计数据[x[区域要素中编号字段名称]]["总生态保护红线用地面积"])
                x["总村庄建设边界用地面积"] = 字类.转换_到字(统计数据[x[区域要素中编号字段名称]]["总村庄建设边界用地面积"])
                x["允许提高幅度"] = 字类.转换_到字(5)

...
class ExportToCAD(object):
    ...
    def getParameterInfo(self):
        ...
        需融合字段名称 = bxarcpy.参数类.参数创建("需融合字段名称", "字符串", 是否多个值=False, 是否可用=False, 默认值=配置.地块要素字段映射.地类编号字段名称)._内嵌对象

        需融合字段中需融合的值的列表 = bxarcpy.参数类.参数创建("需融合字段中需融合的值的列表", "字符串", 是否多个值=True, 是否可用=False, 默认值=["1207"])._内嵌对象
    ...
    def updateParameters(self, 参数列表):
        ...
        if bxarcpy.参数类.值读取(参数字典["是否对要素进行融合"]):
            bxarcpy.参数类.可用性设置(参数字典["需融合字段名称"], True)
            bxarcpy.参数类.可用性设置(参数字典["需融合字段中需融合的值的列表"], True)
        else:
            bxarcpy.参数类.可用性设置(参数字典["需融合字段名称"], False)
            bxarcpy.参数类.可用性设置(参数字典["需融合字段中需融合的值的列表"], False)

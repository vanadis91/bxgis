...
def 层次分析法计算规划站点(
    ...
    if 现状与规划差额字段名称:
        要素类.字段添加(行政界线要素路径, 现状与规划差额字段名称, 字段类型="双精度")
    ...
    # 权重计算
    操作字段 = ["街道名称", "常住人口", "建成区面积", "区生产总值", "区县名称", "区城镇化率", 现状站点数量字段名称, 规划站点数量字段名称, 规划站点数量理论字段名称, 规划站点分配权重字段名称]
    if 现状与规划差额字段名称:
        操作字段.append(现状与规划差额字段名称)
    with 游标类.游标创建("更新", 行政界线要素路径, 操作字段) as 游标:
        ...
        for x in 游标类.属性获取_数据_字典形式(游标, 操作字段):
            ...
            人口密度列表.append(人口密度)
        游标类.重置(游标)
        日志生成器.输出调试(f"总生产总值字典：{总生产总值temp}")
        日志生成器.输出调试(f"总总常住人口：{总常住人口}")
        日志生成器.输出调试(f"总总建成区面积：{总建成区面积}")
        ...
        for i, x in enumerate(游标类.属性获取_数据_字典形式(游标, 操作字段)):
            权重值_未归一 = (x["常住人口"] / 总常住人口 * 0.25 + x["建成区面积"] / 总建成区面积 * 0.2 + 人口密度列表[i] / 总人口密度 * 0.3) * ((x["区生产总值"] / 总生产总值) ** 0.5) * (x["区城镇化率"] ** 0.5 / 10)
        ...
        for i, x in enumerate(游标类.属性获取_数据_字典形式(游标, 操作字段)):
            if 日志生成器.属性获取_当前函数内日志开启状态():
                if x["街道名称"] == "望江街道":
                    日志生成器.输出调试并暂停(f"{x['街道名称']}的现状站点数量是：{x[现状站点数量字段名称]}")
                    日志生成器.输出调试并暂停(f"{x['街道名称']}的理论规划站点数量是：{规划站点数量列表_理论[i]}")
        ...
        while sum(规划站点数量列表_实际_需新增街道_整数) != 站点总数 and 次数 < 1000:
            ...
            规划站点数量列表_实际_需新增街道_整数 = [round(x) for x in 规划站点数量列表_实际_需新增街道_浮点]
            日志生成器.输出调试并暂停(f"迭代后的站点总数：浮点，{sum(规划站点数量列表_实际_需新增街道_浮点)}，整数，{sum(规划站点数量列表_实际_需新增街道_整数)}")
        ...
        日志生成器.输出信息(f"现状和规划站点数量元祖{list(zip(街道名称, 现状站点数量列表, 规划站点数量列表_理论, 规划站点数量列表_实际_需新增街道,站点新建差额))}", 内容长度=20000)
        ...
        for i, x in enumerate(游标类.属性获取_数据_字典形式(游标, 操作字段)):
            ...
            if 现状与规划差额字段名称:
                x[现状与规划差额字段名称] = 规划站点数量列表_实际[i] - 现状站点数量列表[i]
            游标类.行更新_字典形式(游标, x)
    输出要素 = 要素类.要素创建_通过更新(去掉乡之前的行政界线要素路径, 行政界线要素路径)
    if 现状与规划差额字段名称:
        要素类.字段排序(输出要素, [现状站点数量字段名称, 规划站点分配权重字段名称, 规划站点数量理论字段名称, 规划站点数量字段名称, 现状与规划差额字段名称])
    else:

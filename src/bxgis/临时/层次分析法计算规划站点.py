...
def 层次分析法计算规划站点(
    ...
    操作字段 = ["常住人口", "建成区面积", "区生产总值", "区县名称", 现状站点数量字段名称, 规划站点数量字段名称, 规划站点数量理论字段名称, 规划站点分配权重字段名称]
    with 游标类.游标创建("更新", 行政界线要素路径, 操作字段) as 游标:
        ...
        for i, x in enumerate(游标类.属性获取_数据_字典形式(游标, 操作字段)):
            ...
            if 现状站点数量和至少站点数量的大值 >= 规划站点数量列表_理论[i]:
                ...
                维持现状站点数量列表.append(0.0)
                新增站点比例列表.append(权重值列表_归一[i])
                现状站点数量列表.append(x[现状站点数量字段名称])
        游标类.重置(游标)
        ...
        if 新增部分的站点数量 < 0:
            ...
            新增部分的站点数量 = 0
            print("现状站点已超规划目标")
        规划站点数量列表_实际_需新增街道_浮点 = [新增部分的站点数量 * x for x in 权重值列表_再次归一]
        规划站点数量列表_实际_需新增街道_整数 = [round(x) for x in 规划站点数量列表_实际_需新增街道_浮点]

        次数 = 0
        日志类.输出调试(f"新增部分的站点数量：{新增部分的站点数量}")
        差值 = (新增部分的站点数量 - sum(规划站点数量列表_实际_需新增街道_整数)) / len(权重值列表_再次归一)
        差值符号 = "正" if 差值 >= 0 else "负"
        while sum(规划站点数量列表_实际_需新增街道_整数) != 新增部分的站点数量 and 次数 < 1000:
            次数 += 1
            日志类.输出调试(f"差值，第{次数}次：{新增部分的站点数量 - sum(规划站点数量列表_实际_需新增街道_整数)}")
            if 差值 >= 0 and 差值符号 == "正" or 差值 < 0 and 差值符号 == "负":
                差值 = 差值
            else:
                差值 = 差值 / 10
                差值符号 = "负" if 差值 == "正" else "正"
            规划站点数量列表_实际_需新增街道_浮点 = [x + 差值 for x in 规划站点数量列表_实际_需新增街道_浮点]
            规划站点数量列表_实际_需新增街道_整数 = [round(x) for x in 规划站点数量列表_实际_需新增街道_浮点]
            差值 = (新增部分的站点数量 - sum(规划站点数量列表_实际_需新增街道_整数)) / len(权重值列表_再次归一)
        规划站点数量列表_实际_需新增街道 = 规划站点数量列表_实际_需新增街道_整数
        日志类.输出调试(f"现状和规划站点数量元祖{list(zip(维持现状站点数量列表, 规划站点数量列表_实际_需新增街道))}")
        规划站点数量列表_实际 = [int(round(sum(x))) for x in zip(维持现状站点数量列表, 规划站点数量列表_实际_需新增街道)]
        for i, x in enumerate(游标类.属性获取_数据_字典形式(游标, 操作字段)):
...
if __name__ == "__main__":
    ...
    临时工作空间 = r"C:\Users\beixiao\Project\H杭州市户外劳动者服务站点研究\杭州市户外劳动者服务站点研究_数据库_临时.gdb"
    with 环境管理器类.环境管理器类创建(工作空间, 临时工作空间=临时工作空间):

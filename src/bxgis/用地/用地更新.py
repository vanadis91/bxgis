    ...
    with 游标类.游标创建("查询", 地块要素, 需操作的字段名称列表) as 游标_地块:
        for 地块x in 游标类.属性获取_数据_字典形式(游标_地块, 需操作的字段名称列表):
            if 地块x[基本信息.地块要素字段映射.地块编号字段名称] not in ["", " ", None] and 地块x[基本信息.地块要素字段映射.地类编号字段名称][0:2] in ["07", "08", "09", "10", "11", "12", "13", "14", "15", "16"] and 地块x[基本信息.地块要素字段映射.地类编号字段名称][0:4] not in ["1207"]:
                ...
                单元序号 = 字类.匹配正则(地块x[基本信息.地块要素字段映射.地块编号字段名称], 地块编号部件正则_枚举.单元序号正则, "查找所有匹配并返回列表")[0]  # type: ignore
                街区序号 = 字类.匹配正则(地块x[基本信息.地块要素字段映射.地块编号字段名称], 地块编号部件正则_枚举.街区序号正则, "查找所有匹配并返回列表")[0]  # type: ignore
                地块序号 = 字类.匹配正则(地块x[基本信息.地块要素字段映射.地块编号字段名称], 地块编号部件正则_枚举.地块序号正则, "查找所有匹配并返回列表")[0]  # type: ignore

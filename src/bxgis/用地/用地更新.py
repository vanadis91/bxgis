...
def 用地更新(
    ...
    if 是否进行合规性检查:
        ...
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, 输出要素路径 + "temp")
    ...
    with 游标类.游标创建("更新", 输入要素, 操作字段列表) as 游标:
        for 要素x in 游标类.属性获取_数据_字典形式(游标, 操作字段列表):
            ...
            if len(要素x["实体GUID"].split("-")) != 5:
                输入输出类.输出消息(f"GUID错误：{要素x['_ID']}，GUID：{要素x['实体GUID']}")
                要素x["实体GUID"] = 字类.字符串生成_GUID()
            if 要素x["实体GUID"] in 既有GUID列表:
                输入输出类.输出消息(f"GUID重复：{要素x['_ID']}，GUID：{要素x['实体GUID']}")
                要素x["实体GUID"] = 字类.字符串生成_GUID()
...
def 计算开发动态(输入要素名称, 城镇集建区要素名称="KZX_城镇集建区", 城镇弹性区要素名称="KZX_城镇弹性区", 输出要素名称="in_memory\\AA_计算开发动态"):
    ...
    with 游标类.游标创建("更新", 用地要素, ["所属三线", 基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.开发动态字段名称]) as 游标:
        for x in 游标类.属性获取_数据_字典形式(游标, ["所属三线", 基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.开发动态字段名称]):
            ...
            if x["开发动态"] in ["现状已实施", "现状保留", "保留"] and x["所属三线"] == "开发边界内":
                x["开发动态"] = 基本信息.开发动态状态_枚举.开发边界内保留
            elif x["开发动态"] in ["现状已实施", "现状保留", "保留"] and x["所属三线"] != "开发边界内":

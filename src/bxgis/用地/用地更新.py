...
def 用地更新(输入要素名称="DIST_用地规划图", 街坊范围线要素名称="JX_街坊范围线", 分村范围线要素名称="JX_分村范围线", 城镇集建区要素名称="KZX_城镇集建区", 城镇弹性区要素名称="KZX_城镇弹性区", 有扣除地类系数的要素名称="CZ_三调筛选_扣除地类系数", 有坐落单位信息的要素名称="CZ_三调筛选_坐落单位名称", 输出要素名称="in_memory\\AA_用地更新"):
    if 输出要素名称 == "in_memory\\AA_用地更新":
    ...
    if 街坊范围线要素名称:
        用地要素 = 常用.计算所属区域(用地要素.名称, 区域要素名称=街坊范围线要素名称, 字段映射列表=[[配置.地块要素字段映射.所属街坊字段名称, 配置.区域要素字段映射.区域编号字段名称]], 计算方式="分割输入要素")
        用地要素 = 根据所属街坊生成所属街区(用地要素.名称)
    if 分村范围线要素名称:
        用地要素 = 常用.计算所属区域(用地要素.名称, 区域要素名称=分村范围线要素名称, 字段映射列表=[[配置.地块要素字段映射.所属分村字段名称, 配置.区域要素字段映射.区域编号字段名称]], 计算方式="分割输入要素")
    用地要素 = 计算地块编号(用地要素.名称)
...
def 计算耕保量(输入要素名称, 有扣除地类系数的要素名称="CZ_三调筛选_扣除地类系数", 输出要素名称="in_memory\\AA_计算耕保量"):
    ...
    with bxarcpy.游标类.游标创建_通过名称("更新", 合并扣除系数后要素.名称, [配置.地块要素字段映射.地类编号字段名称, "扣除地类系数", "SHAPE@AREA", 配置.地块要素字段映射.耕地保有量字段名称]) as 游标:
        for x in 游标:
            if 字.匹配正则(x[配置.地块要素字段映射.地类编号字段名称], "^01"):
...
def 根据地类编号生成名称(输入要素名称, 地类编号字段名称=配置.地块要素字段映射.地类编号字段名称, 性质名称字段名称=配置.地块要素字段映射.性质名称字段名称, 地块性质别称字段名称=配置.地块要素字段映射.地块性质别称字段名称, 用地构成字段名称=配置.地块要素字段映射.用地大类字段名称, 输出要素名称="in_memory\\AA_根据地类编号生成名称"):
    ...
    当前文件所在目录 = 系统.路径.部件_目录(__file__)
    转换文件路径 = 系统.路径.转绝对("..\\", 当前文件所在目录)
    ...
    with bxarcpy.游标类.游标创建_通过名称("更新", 输入要素.名称, [地类编号字段名称, 性质名称字段名称, 地块性质别称字段名称, 用地构成字段名称]) as 游标:
        for x in 游标:
            ...
            if 对应的对象列表:
                ...
                print(f"未找到 {x[地类编号字段名称]} 对应的 性质名称。")
                部分地类未找到名称flag = True
    if 部分地类未找到名称flag:
    ...
    with bxarcpy.游标类.游标创建_通过名称("更新", 有地籍地块要素.名称, 需操作的字段名称列表) as 游标_地块:
        for x in 游标_地块:
            ...
            if x[配置.地块要素字段映射.地块编号字段名称] not in ["", " ", None] and len(x[配置.地块要素字段映射.地块编号字段名称].split("-")) == 2 and len(x[配置.地块要素字段映射.地块编号字段名称].split("-")[0]) == 8:
                ...
                地块编号 = 字.格式_补位(x[配置.地块要素字段映射.地块编号字段名称].split("-")[-1], 3)
...
def 计算开发动态(输入要素名称, 城镇集建区要素名称="KZX_城镇集建区", 城镇弹性区要素名称="KZX_城镇弹性区", 输出要素名称="in_memory\\AA_计算开发动态"):
    ...
    用地要素 = 常用.计算所属区域(输入要素名称=输入要素.名称, 区域要素名称=开发边界要素.名称, 字段映射列表=[["所属三线", "控制线名称"]], 计算方式="内点在区域要素内")
...
def 计算地块内设施规模(输入要素名称="DIST_用地规划图", 设施要素名称="SS_配套设施", 输出要素名称="内存临时"):
    ...
    用地要素.字段添加(配置.地块要素字段映射.配套设施规模字段名称)
    with bxarcpy.游标类.游标创建_通过名称("更新", 用地要素.名称, [配置.地块要素字段映射.配套设施规模字段名称]) as 用地游标:
        for 用地x in 用地游标:
            pass
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        用地更新(输入要素名称="DIST_用地规划图", 街坊范围线要素名称="JX_街坊范围线", 分村范围线要素名称="JX_分村范围线", 城镇集建区要素名称="KZX_城镇集建区", 城镇弹性区要素名称="KZX_城镇弹性区", 有扣除地类系数的要素名称="CZ_三调筛选_扣除地类系数", 有坐落单位信息的要素名称="CZ_三调筛选_坐落单位名称", 输出要素名称="DIST_用地规划图_更新后")

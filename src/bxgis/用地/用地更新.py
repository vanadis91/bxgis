...
def 用地更新(
    ...
    if 日志类.属性获取_当前函数内日志开启状态():
        要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_完成了所属街区街坊分村的计算")
...
def 计算耕保量(输入要素名称, 有扣除地类系数的要素名称="CZ_三调筛选_扣除地类系数", 输出要素名称="in_memory\\AA_计算耕保量"):
    ...
    要素类.字段修改(带有扣除系数的耕地要素, "扣除地类系数", "扣除地类系数_1")
    if 日志类.属性获取_当前函数内日志开启状态():
        要素类.要素创建_通过复制并重命名重名要素(带有扣除系数的耕地要素, "AA_带有扣除系数的耕地要素")
        日志类.输出调试并暂停("带有扣除系数的耕地要素")

    要素类.字段添加(用地要素, "扣除地类系数", "双精度")
    合并扣除系数后要素 = 要素类.要素创建_通过联合并赋值字段(用地要素, 带有扣除系数的耕地要素, [["扣除地类系数", "扣除地类系数_1"]], 要素被分割时提示信息中包括的字段=[基本信息.地块要素字段映射.地类编号字段名称])
    if 日志类.属性获取_当前函数内日志开启状态():

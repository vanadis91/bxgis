...
def 用地数据合规性检查(用地要素名称):
    ...
    for x in 需要存在的字段名称列表:
        if x not in 字段名称列表:
            部分字段不存在flag = True
            输入输出类.输出消息(f"{用地要素名称}中缺少{x}字段")
    if 部分字段不存在flag:
        raise Exception(f"{用地要素名称}中缺少{x}字段。")
    ...
    for x in 字段列表:
        if 字段类.属性获取_名称(x) in [
            ...
            if 字段类.属性获取_类型(x) != "String":
                字段类型不准确flag = True
                输入输出类.输出消息(f"{用地要素名称}的{字段类.属性获取_名称(x)}字段请采用定长字符串形式")
    if 字段类型不准确flag:
        raise Exception(f"部分字段的类型不准确。")

    # 检查地类编号是否存在空值
    地类编号存在空值的要素 = 要素类.要素创建_通过筛选(用地要素, f"{基本信息.地块要素字段映射.地类编号字段名称} IS NULL")
    if 要素类.属性获取_几何数量(地类编号存在空值的要素) > 0:
        输入输出类.输出消息(f"{用地要素名称}部分地块的地类编号为空")
        raise Exception(f"{用地要素名称}部分地块的地类编号为空")
    return 用地要素
...
def 融合部分地类(输入要素名称, 城镇集建区要素名称="KZX_城镇集建区", 城镇弹性区要素名称="KZX_城镇弹性区", 输出要素名称="in_memory\\AA_融合部分地类"):
    ...
    要素类.字段删除(temp1, 保留字段名称列表=输入要素字段名称列表)
    仅河道要素 = 要素类.要素创建_通过更新并合并字段(仅河道要素, temp1)
    输入要素 = 要素类.要素创建_通过更新并合并字段(仅河道要素, 仅河道要素)
...
def 根据所属街坊生成所属街区(输入要素名称, 输出要素名称="in_memory\\AA_根据所属街坊生成所属街区"):
    ...
    with 游标类.游标创建("更新", 输入要素, [基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.地块要素字段映射.所属街区字段名称]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, [基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.地块要素字段映射.所属街区字段名称]):
...
def 计算地块编号(输入要素名称, 输出要素名称="in_memory\\AA_计算地块编号"):
    ...
    with 游标类.游标创建("查询", 地块要素, 需操作的字段名称列表) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, 需操作的字段名称列表):
    ...
    with 游标类.游标创建("更新", 地块要素, [基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.地块编号字段名称, 基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.地块要素字段映射.所属分村字段名称, "_ID"]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, [基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.地块编号字段名称, 基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.地块要素字段映射.所属分村字段名称, "_ID"]):
            地块编号序号 = 1
    ...
    with 游标类.游标创建("更新", 仅河道要素, [基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.地块编号字段名称, 基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.地块要素字段映射.所属分村字段名称, "_ID"]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, [基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.地块编号字段名称, 基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.地块要素字段映射.所属分村字段名称, "_ID"]):
            地块编号序号 = 1
            if x[基本信息.地块要素字段映射.地块编号字段名称] in ["", " ", None] and x[基本信息.地块要素字段映射.地类编号字段名称][0:2] in ["17"]:
                if x[基本信息.地块要素字段映射.所属街坊字段名称] not in ["", " ", None]:
                    字典类.默认值设置(地块编号字典, [基本信息.地块要素字段映射.所属街坊字段名称], [])
                    while 地块编号序号 in 地块编号字典[基本信息.地块要素字段映射.所属街坊字段名称]:
                        地块编号序号 += 1
    ...
    with 游标类.游标创建("更新", 非建设用地要素, [基本信息.地块要素字段映射.地块编号字段名称, 基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.地块要素字段映射.所属分村字段名称, "_ID"]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, [基本信息.地块要素字段映射.地块编号字段名称, 基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.地块要素字段映射.所属分村字段名称, "_ID"]):
            if x[基本信息.地块要素字段映射.所属街坊字段名称] not in ["", " ", None]:
...
def 计算耕保量(输入要素名称, 有扣除地类系数的要素名称="CZ_三调筛选_扣除地类系数", 输出要素名称="in_memory\\AA_计算耕保量"):
    ...
    要素类.字段删除(带有扣除系数的耕地要素, 保留字段名称列表=["扣除地类系数"])

    合并扣除系数后要素 = 要素类.要素创建_通过联合([用地要素, 带有扣除系数的耕地要素], 是否保留FID=False)
    ...
    耕地保有量汇总 = 0
    with 游标类.游标创建("更新", 合并扣除系数后要素, [基本信息.地块要素字段映射.地类编号字段名称, "扣除地类系数", "SHAPE@AREA", 基本信息.地块要素字段映射.耕地保有量字段名称]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, [基本信息.地块要素字段映射.地类编号字段名称, "扣除地类系数", "SHAPE@AREA", 基本信息.地块要素字段映射.耕地保有量字段名称]):
...
def 根据地类编号生成名称(输入要素名称, 地类编号字段名称=基本信息.地块要素字段映射.地类编号字段名称, 性质名称字段名称=基本信息.地块要素字段映射.性质名称字段名称, 地块性质别称字段名称=基本信息.地块要素字段映射.地块性质别称字段名称, 用地构成字段名称=基本信息.地块要素字段映射.用地大类字段名称, 输出要素名称="内存临时"):
    if 输出要素名称 == "内存临时":
    ...
    a = 数据框架类.转换_从excel文件(转换文件路径 + r"\config\地块_指标测算表.xlsx", 要读取的列=[1, 3, 4, 5, 86], 指定数据类型={"性质名称": str, "地块性质": str, "地类标准": str, "地块性质别称": str, "用地构成": str})
    ...
    with 游标类.游标创建("更新", 输入要素, [地类编号字段名称, 性质名称字段名称, 地块性质别称字段名称, 用地构成字段名称]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, [地类编号字段名称, 性质名称字段名称, 地块性质别称字段名称, 用地构成字段名称]):
...
def 计算土地码(输入要素名称, 地籍要素名称="CZ_三调筛选_坐落单位名称", 输出要素名称="in_memory\\AA_计算土地码"):
    ...
    with 游标类.游标创建("更新", 有地籍地块要素, 需操作的字段名称列表) as 游标_地块:
        for x in 游标类.游标迭代器转换_字典形式(游标_地块, 需操作的字段名称列表):
            地籍代码 = "H00000000"
            主地类编号 = "00000000"
            兼容性质数量 = 1
            街区编号 = "00"
...
def 计算开发动态(输入要素名称, 城镇集建区要素名称="KZX_城镇集建区", 城镇弹性区要素名称="KZX_城镇弹性区", 输出要素名称="in_memory\\AA_计算开发动态"):
    ...
    with 游标类.游标创建("更新", 用地要素, ["所属三线", 基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.开发动态字段名称]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, ["所属三线", 基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.开发动态字段名称]):
...
def 计算地块内设施规模(输入要素名称="DIST_用地规划图", 设施要素名称="SS_配套设施", 输出要素名称="内存临时"):
    ...
    要素类.字段添加(用地要素, 基本信息.地块要素字段映射.配套设施规模字段名称)
    要素类.字段添加(用地要素, 基本信息.地块要素字段映射.配套设施规模字段名称 + "2")
    要素类.字段添加(用地要素, 基本信息.地块要素字段映射.配套设施规模字段名称 + "3")
    使用到了配套设施规模2字段 = False
    使用到了配套设施规模3字段 = False
    with 游标类.游标创建("更新", 用地要素, ["_ID", "_形状", 基本信息.地块要素字段映射.配套设施规模字段名称, 基本信息.地块要素字段映射.配套设施规模字段名称 + "2", 基本信息.地块要素字段映射.配套设施规模字段名称 + "3"]) as 用地游标:
        with 游标类.游标创建("查询", 设施要素, ["_形状", 基本信息.设施要素字段映射.设施名称字段名称, 基本信息.设施要素字段映射.设施规模字段名称]) as 设施游标:
            for 用地x in 用地游标:
                地块设施规模内容 = []
                for 设施x in 设施游标:

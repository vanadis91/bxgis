...
from bxarcpy.环境包 import 环境管理器类, 输入输出类, 环境类
...
def 用地更新(
    ...
    是否计算机动车开口=True,
    开口符号要素路径="YD_TZ_机动车开口方位",
    是否输出更新前后变化日志=True,
    日志输出路径: Union[Literal[r"C:\Users\beixiao\Desktop\debug_用地更新.txt"], Any] = None,
    输出要素路径="内存临时",
):
    ...
    日志生成器.临时关闭日志()
    if 日志输出路径 is not None:
        日志处理器.格式化器_内容着色 = False
        日志处理器.输出器_文件对象_路径 = 日志输出路径
        日志生成器.临时开启日志
    ...
    if 局部更新SQL语句 == "":
        ...
        用地要素_全 = 要素类.要素创建_通过复制(用地要素路径)
        用地要素 = 要素类.要素创建_通过筛选(用地要素路径, 局部更新SQL语句)

    if "AA_用地更新010_通过了数据合规性检查" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新010_通过了数据合规性检查】，是否跳过合规性检查 [是(y)/否(n)]：")
        是否进行合规性检查 = False if 输入内容.upper() == "Y" else 是否进行合规性检查
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新010_通过了数据合规性检查")
    if 是否进行合规性检查:
        输入输出类.输出消息("开始合规性检查")

        用地要素 = 用地合规性检查.用地合规性检查(用地要素)
        数据检查.数据检查(有坐落单位信息的要素名称, 是否几何修复=False)
        if 街坊范围线要素名称:
            数据检查.数据检查(街坊范围线要素名称, 是否几何修复=False)
        if 分村范围线要素名称:
            数据检查.数据检查(分村范围线要素名称, 是否几何修复=False)
        数据检查.数据检查(城镇集建区要素名称, 是否几何修复=False, 是否范围检查=False)
        数据检查.数据检查(城镇弹性区要素名称, 是否几何修复=False, 是否范围检查=False)
        数据检查.数据检查(有扣除地类系数的要素名称, 是否几何修复=False, 是否范围检查=False)
        数据检查.数据检查(用地要素, 是否几何修复=False)

        输入输出类.输出消息("通过了数据合规性检查")
        要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新010_通过了数据合规性检查")

    if "AA_用地更新020_完成了用地边界的调整" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新020_完成了用地边界的调整】，是否跳过用地边界的调整 [是(y)/否(n)]：")
        是否对用地边界进行调整 = False if 输入内容.upper() == "Y" else 是否对用地边界进行调整
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新020_完成了用地边界的调整")
    if 是否对用地边界进行调整:
        输入输出类.输出消息("开始融合部分地类")
        用地要素 = 融合部分地类(用地要素)
        ...
        if 街坊范围线要素名称:
            用地要素 = 属性更新.常用_属性更新(用地要素, 区域要素名称=街坊范围线要素名称, 字段映射列表=[[基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.区域要素字段映射.区域编号字段名称]], 计算方式="内点在区域要素内")
            用地要素 = 根据所属街坊生成所属街区(用地要素)
        else:
            要素类.字段添加(用地要素, 基本信息.地块要素字段映射.所属街坊字段名称)

        if 分村范围线要素名称:
            用地要素 = 属性更新.常用_属性更新(用地要素, 区域要素名称=分村范围线要素名称, 字段映射列表=[[基本信息.地块要素字段映射.所属分村字段名称, 基本信息.区域要素字段映射.区域编号字段名称]], 计算方式="内点在区域要素内")
        else:
        ...
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新020_完成了用地边界的调整")

    if "AA_用地更新025_完成了重新生成GUID" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新025_完成了重新生成GUID】，是否跳过重新生成GUID [是(y)/否(n)]：")
        是否重新生成GUID = False if 输入内容.upper() == "Y" else 是否重新生成GUID
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新025_完成了重新生成GUID")
        用地要素副本 = 要素类.要素创建_通过复制("AA_用地更新025_完成了重新生成GUID")
    if 是否重新生成GUID:
        用地要素 = GUID唯一性检查(用地要素)
        输入输出类.输出消息("完成了重新生成GUID")
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新025_完成了重新生成GUID")
        用地要素副本 = 要素类.要素创建_通过复制(用地要素)

    if "AA_用地更新030_完成了所属街坊分村的计算" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新030_完成了所属街坊分村的计算】，是否跳过所属街坊分村的计算 [是(y)/否(n)]：")
        是否重新计算所属街区街坊和分村 = False if 输入内容.upper() == "Y" else 是否重新计算所属街区街坊和分村
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新030_完成了所属街坊分村的计算")
    if 是否重新计算所属街区街坊和分村:
        if 街坊范围线要素名称:
            用地要素 = 属性更新.常用_属性更新(用地要素, 区域要素名称=街坊范围线要素名称, 字段映射列表=[[基本信息.地块要素字段映射.所属街坊字段名称, 基本信息.区域要素字段映射.区域编号字段名称]], 计算方式="内点在区域要素内")
            用地要素 = 根据所属街坊生成所属街区(用地要素)
        ...
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新030_完成了所属街坊分村的计算")

    if "AA_用地更新040_完成了耕保量的计算" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新040_完成了耕保量的计算】，是否跳过耕保量的计算 [是(y)/否(n)]：")
        是否重新计算耕保量 = False if 输入内容.upper() == "Y" else 是否重新计算耕保量
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新040_完成了耕保量的计算")
    if 是否重新计算耕保量:
        用地要素 = 计算耕保量(用地要素, 有扣除地类系数的要素路径=有扣除地类系数的要素名称)
        if 日志生成器.属性获取_当前函数内日志开启状态():
            要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_完成了耕保量的计算")
        输入输出类.输出消息("完成了耕保量的计算")
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新040_完成了耕保量的计算")

    if "AA_用地更新050_完成了根据地类编号生成名称" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新050_完成了根据地类编号生成名称】，是否跳过根据地类编号生成名称 [是(y)/否(n)]：")
        是否根据地类编号生成地类名称 = False if 输入内容.upper() == "Y" else 是否根据地类编号生成地类名称
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新050_完成了根据地类编号生成名称")
    if 是否根据地类编号生成地类名称:
        用地要素 = 根据地类编号生成名称(用地要素)
        if 日志生成器.属性获取_当前函数内日志开启状态():
            要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_完成了根据地类编号生成名称")
        输入输出类.输出消息("完成了根据地类编号生成名称")
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新050_完成了根据地类编号生成名称")

    if "AA_用地更新060_完成了地块编号的计算" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新060_完成了地块编号的计算】，是否跳过地块编号的计算 [是(y)/否(n)]：")
        是否计算地块编号 = False if 输入内容.upper() == "Y" else 是否计算地块编号
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新060_完成了地块编号的计算")
    if 是否计算地块编号:
        用地要素 = 计算地块编号(用地要素, 城镇集建区要素名称, 城镇弹性区要素名称, 开发边界内非建设用地是否编号=开发边界内非建设用地是否编号, 道路是否编号=道路是否编号, 开发边界外非建设用地是否编号=开发边界外非建设用地是否编号)
        if 日志生成器.属性获取_当前函数内日志开启状态():
            要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_完成了地块编号的计算")
        输入输出类.输出消息("完成了地块编号的计算")
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新060_完成了地块编号的计算")
    ...
    if "AA_用地更新070_完成了土地码的计算" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新070_完成了土地码的计算】，是否跳过土地码的计算 [是(y)/否(n)]：")
        是否计算土地码 = False if 输入内容.upper() == "Y" else 是否计算土地码
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新070_完成了土地码的计算")
    if 是否计算土地码:
        用地要素 = 计算土地码(用地要素, 地籍要素名称=有坐落单位信息的要素名称)
        if 日志生成器.属性获取_当前函数内日志开启状态():
            要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_完成了土地码的计算")
        输入输出类.输出消息("完成了土地码的计算")
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新070_完成了土地码的计算")

    if "AA_用地更新080_完成了开发动态的计算" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新080_完成了开发动态的计算】，是否跳过开发动态的计算 [是(y)/否(n)]：")
        是否计算开发动态 = False if 输入内容.upper() == "Y" else 是否计算开发动态
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新080_完成了开发动态的计算")
    if 是否计算开发动态:
        用地要素 = 计算开发动态(用地要素)
        if 日志生成器.属性获取_当前函数内日志开启状态():
            要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_完成了开发动态的计算")
        输入输出类.输出消息("完成了开发动态的计算")
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新080_完成了开发动态的计算")

    if "AA_用地更新090_完成了配套设施的计算" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新090_完成了配套设施的计算】，是否跳过配套设施的计算 [是(y)/否(n)]：")
        是否计算地块内设施规模 = False if 输入内容.upper() == "Y" else 是否计算地块内设施规模
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新090_完成了配套设施的计算")
    if 是否计算地块内设施规模:
        if 设施要素名称:
        ...
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新090_完成了配套设施的计算")

    if "AA_用地更新100_完成了其他属性的计算" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新100_完成了其他属性的计算】，是否跳过其他属性的计算 [是(y)/否(n)]：")
        是否计算建筑规模市政规模等其他属性 = False if 输入内容.upper() == "Y" else 是否计算建筑规模市政规模等其他属性
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新100_完成了其他属性的计算")
    if 是否计算建筑规模市政规模等其他属性:
        用地要素 = 计算地块其他属性(用地要素)
        if 日志生成器.属性获取_当前函数内日志开启状态():
            要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_完成了其他属性的计算")
        输入输出类.输出消息("完成了其他属性的计算")
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新100_完成了其他属性的计算")

    if "AA_用地更新110_完成了机动车开口的计算" in 要素名称列表:
        输入内容 = input("已经存在【AA_用地更新110_完成了机动车开口的计算】，是否跳过机动车开口的计算 [是(y)/否(n)]：")
        是否计算机动车开口 = False if 输入内容.upper() == "Y" else 是否计算机动车开口
        用地要素 = 要素类.要素创建_通过复制("AA_用地更新110_完成了机动车开口的计算")
    if 是否计算机动车开口:
        用地要素 = 计算开口方位(用地要素, 开口符号要素路径)
        输入输出类.输出消息("完成了机动车开口的计算")
        用地要素 = 要素类.要素创建_通过复制并重命名重名要素(用地要素, "AA_用地更新110_完成了机动车开口的计算")

    if 是否输出更新前后变化日志:
        from bxgis.属性.属性对比 import 属性对比

        字段名称列表 = 要素类.字段名称列表获取(用地要素, 含系统字段=False)
        对比字段名称列表 = [[x, x] for x in 字段名称列表]
        属性对比(
            输入要素路径1=用地要素副本,
            输入要素路径2=用地要素,
            映射字段名称列表=["实体GUID", "实体GUID"],
            对比字段名称列表=对比字段名称列表,
            转换为数字后进行对比的字段属性列表=[{"字段名称": "耕地保有量", "小数点后位数": 2}, {"字段名称": "面积指定", "小数点后位数": 2}, {"字段名称": "面积公顷", "小数点后位数": 2}, {"字段名称": "面积", "小数点后位数": 2}, {"字段名称": "绿地面积", "小数点后位数": 2}],
        )

    用地要素 = 数据检查.数据检查(用地要素, 是否几何修复=False)
    if 局部更新SQL语句 != "":
        用地要素 = 要素类.要素创建_通过更新并合并字段(用地要素_全, 用地要素, False)
...
def GUID唯一性检查(输入要素路径, 输出要素路径="内存临时"):
    日志生成器.临时关闭日志()
    from bxpy.基本对象包 import 字类, 字典类, 浮类

    输出要素路径 = 工具包.输出路径生成_当采用内存临时时(["GUID唯一性检查"]) if 输出要素路径 == "内存临时" else 输出要素路径
    输入要素 = 要素类.要素创建_通过复制(输入要素路径)
    操作字段列表 = ["_ID", "实体GUID"]
    既有GUID列表 = []

    with 游标类.游标创建("更新", 输入要素, 操作字段列表) as 游标:
        for 要素x in 游标类.属性获取_数据_字典形式(游标, 操作字段列表):
            if 要素x["实体GUID"] in ["", " ", None]:
                输入输出类.输出消息(f"GUID为空：{要素x['_ID']}，GUID：{要素x['实体GUID']}")
                要素x["实体GUID"] = 字类.字符串生成_GUID()
            if len(要素x["实体GUID"].split("-")) == 6:
                要素x["实体GUID"] = "-".join(要素x["实体GUID"].split("-")[0:5])
            if len(要素x["实体GUID"].split("-")) != 5:
                a = 字类.字符串生成_GUID()
                输入输出类.输出消息(f"GUID错误：{要素x['_ID']}，GUID：{要素x['实体GUID']}，修改为：{a}")
                要素x["实体GUID"] = a
            if 要素x["实体GUID"] in 既有GUID列表:
                a = 字类.字符串生成_GUID()
                输入输出类.输出消息(f"GUID重复：{要素x['_ID']}，GUID：{要素x['实体GUID']}，修改为：{a}")
                要素x["实体GUID"] = a
            既有GUID列表.append(要素x["实体GUID"])
            游标类.行更新_字典形式(游标, 要素x)
...
def 计算耕保量(输入要素名称, 有扣除地类系数的要素路径=基本信息.项目信息.CZ_三调_扣除地类系数, 输出要素路径="内存临时"):
    日志生成器.临时关闭日志()
    输出要素路径 = 工具包.临时路径生成(["计算耕保量"]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    合并扣除系数后耕地要素 = 要素类.要素创建_通过联合并赋值字段(仅有耕地的要素, 带有扣除系数的耕地要素, [["扣除地类系数", "扣除地类系数"]], 要素被分割时提示信息中包括的字段=[基本信息.地块要素字段映射.地类编号字段名称], 是否多部件转单部件=True)
    if 日志生成器.属性获取_当前函数内日志开启状态():
...
def 计算地块编号(输入要素路径, 城镇集建区要素路径="KZX_城镇集建区", 城镇弹性区要素路径="KZX_城镇弹性区", 开发边界内非建设用地是否编号=True, 道路是否编号=True, 开发边界外非建设用地是否编号=True, 输出要素路径="内存临时"):
    日志生成器.临时关闭日志()
    输出要素路径 = 工具包.输出路径生成_当采用内存临时时(["道路合并成多部件"]) if 输出要素路径 == "内存临时" else 输出要素路径
    # 需要保证所有地块都具有所属街区和所属街坊两个字段并赋值
    ...
    地块编号字典 = {}
    编号存在重复flag = False
    with 游标类.游标创建("查询", 地块要素, 需操作的字段名称列表) as 游标_地块:
        for 地块x in 游标类.属性获取_数据_字典形式(游标_地块, 需操作的字段名称列表):
            if 地块x[基本信息.地块要素字段映射.地块编号字段名称] not in ["", " ", None] and 地块x[基本信息.地块要素字段映射.地类编号字段名称][0:2] in ["07", "08", "09", "10", "11", "12", "13", "14", "15", "16"] and 地块x[基本信息.地块要素字段映射.地类编号字段名称][0:4] not in ["1207"]:
                from bxgis.配置.基本信息 import 地块编号部件正则_枚举
                from bxpy.基本对象包 import 字类, 字典类
                from bxpy.路径包 import 路径类

                项目信息 = 字典类.转换_从文件(路径类.连接(路径类.属性获取_目录(__file__, 2), "配置", "项目信息.yml"))

                所属区域筛选 = eval(项目信息["地块编号部件筛选_枚举"]["所属区域筛选"])
                所属区域 = 所属区域筛选(地块x[基本信息.地块要素字段映射.地块编号字段名称])
                # 街区序号筛选 = eval(项目信息["地块编号部件筛选_枚举"]["街区序号筛选"])
                # 街区序号 = 街区序号筛选(地块x[基本信息.地块要素字段映射.地块编号字段名称])
                # 街坊序号筛选 = eval(项目信息["地块编号部件筛选_枚举"]["街坊序号筛选"])
                # 街坊序号 = 街坊序号筛选(地块x[基本信息.地块要素字段映射.地块编号字段名称])
                地块所属区域 = str(所属区域)

                地块序号筛选 = eval(项目信息["地块编号部件筛选_枚举"]["地块序号筛选"])
                地块序号 = 地块序号筛选(地块x[基本信息.地块要素字段映射.地块编号字段名称])
                地块编号序号 = int(地块序号)  # type: ignore

                # print(f"地块编号序号：{地块编号序号}，地块所属区域：{地块所属区域}")
                if 地块所属区域 in 地块编号字典 and 地块编号序号 not in 地块编号字典[地块所属区域]:
                    ...
                    编号存在重复flag = True
                    输入输出类.输出消息(f"{地块x[基本信息.地块要素字段映射.地块编号字段名称]}存在重复")
                else:
    ...
    with 游标类.游标创建("更新", 地块要素, 操作字段) as 游标_地块:
        for 地块x in 游标类.属性获取_数据_字典形式(游标_地块, 操作字段):
            地块编号序号 = 1
            if 地块x[基本信息.地块要素字段映射.地块编号字段名称] in ["", " ", None] and 地块x[基本信息.地块要素字段映射.地类编号字段名称][0:2] in ["07", "08", "09", "10", "11", "12", "13", "14", "15", "16"] and 地块x[基本信息.地块要素字段映射.地类编号字段名称][0:4] not in ["1207"]:
                if 地块x[基本信息.地块要素字段映射.所属街坊字段名称] not in ["", " ", None]:
                    字典类.默认值设置(地块编号字典, [地块x[基本信息.地块要素字段映射.所属街坊字段名称]], [])
                    while 地块编号序号 in 地块编号字典[地块x[基本信息.地块要素字段映射.所属街坊字段名称]]:
                        地块编号序号 += 1
                    地块编号字典[地块x[基本信息.地块要素字段映射.所属街坊字段名称]].append(地块编号序号)
                    地块x[基本信息.地块要素字段映射.地块编号字段名称] = 地块x[基本信息.地块要素字段映射.所属街坊字段名称] + "-" + 字类.格式_补位(str(地块编号序号), 2)
                    输入输出类.输出消息(f"ID{地块x['_ID']}的地块被编号为{地块x[基本信息.地块要素字段映射.地块编号字段名称]}")
                    游标类.行更新_字典形式(游标_地块, 地块x)
                elif 地块x[基本信息.地块要素字段映射.所属分村字段名称] not in ["", " ", None]:
                    字典类.默认值设置(地块编号字典, [地块x[基本信息.地块要素字段映射.所属分村字段名称]], [])
                    while 地块编号序号 in 地块编号字典[地块x[基本信息.地块要素字段映射.所属分村字段名称]]:
                        地块编号序号 += 1
                    地块编号字典[地块x[基本信息.地块要素字段映射.所属分村字段名称]].append(地块编号序号)
                    地块x[基本信息.地块要素字段映射.地块编号字段名称] = 地块x[基本信息.地块要素字段映射.所属分村字段名称] + "-" + 字类.格式_补位(str(地块编号序号), 2)
                    输入输出类.输出消息(f"ID{地块x['_ID']}的地块被编号为{地块x[基本信息.地块要素字段映射.地块编号字段名称]}")
                    游标类.行更新_字典形式(游标_地块, 地块x)
                else:
                    是否有地块所属街坊和分村都为空flag = True
                    输入输出类.输出消息(f"ID为{地块x['_ID']}的地块所属街坊和所属分村都为空，无法生成编号。")
    ...
    if 开发边界内非建设用地是否编号:
        ...
        开发边界要素 = 要素类.要素创建_通过合并([城镇集建区要素路径, 城镇弹性区要素路径])

        是否有河道所属街坊和分村都为空flag = False
        非建设用地需操作的字段名称列表 = [地类编号字段名称, 地块编号字段名称, 所属街坊字段名称, 所属分村字段名称, "_ID", "_形状"]
        开发边界需操作的字段名称列表 = ["_形状"]
        with 游标类.游标创建("更新", 非建设用地, 非建设用地需操作的字段名称列表) as 游标_非建设用地, 游标类.游标创建("查询", 开发边界要素, 开发边界需操作的字段名称列表) as 游标_开发边界:
            for 非建设用地x in 游标类.属性获取_数据_字典形式(游标_非建设用地, 非建设用地需操作的字段名称列表):
                是否在开发边界内 = False
                for 开发边界x in 游标类.属性获取_数据_字典形式(游标_开发边界, 开发边界需操作的字段名称列表):
        ...
        日志生成器.输出调试(f"开发边界内非建设用地编号结束后的编号字典：{地块编号字典}", 内容长度=20000)
...
def 计算开发动态(输入要素名称, 城镇集建区要素名称="KZX_城镇集建区", 城镇弹性区要素名称="KZX_城镇弹性区", 输出要素名称="内存临时"):
    输出要素名称 = 工具包.临时路径生成(["计算开发动态"]) if 输出要素名称 == "内存临时" else 输出要素名称
    ...
    用地要素 = 属性更新.常用_属性更新(输入要素, 开发边界要素, 字段映射列表=[["所属三线", 基本信息.控制线要素字段映射.控制线名称字段名称]], 计算方式="内点在区域要素内")

    操作字段名称列表 = ["所属三线", 基本信息.地块要素字段映射.地类编号字段名称, 基本信息.地块要素字段映射.开发动态字段名称]
    with 游标类.游标创建("更新", 用地要素, 操作字段名称列表) as 游标:
        for x in 游标类.属性获取_数据_字典形式(游标, 操作字段名称列表):
            if x["所属三线"] not in ["", " ", None]:
                x["所属三线"] = "开发边界内"

            if x["开发动态"] in ["现状已实施", "现状保留", "保留", "现状"] and x["所属三线"] == "开发边界内":
                x["开发动态"] = 基本信息.开发动态状态_枚举.开发边界内保留
            elif x["开发动态"] in ["现状已实施", "现状保留", "保留", "现状"] and x["所属三线"] != "开发边界内":
                x["开发动态"] = 基本信息.开发动态状态_枚举.开发边界外保留
            elif x["开发动态"] in ["改/扩建", "改扩建", "盘活", "更新", "改造"] and x["所属三线"] == "开发边界内":
                x["开发动态"] = 基本信息.开发动态状态_枚举.开发边界内改造
            elif x["开发动态"] in ["改/扩建", "改扩建", "盘活", "更新", "改造"] and x["所属三线"] != "开发边界内":
                x["开发动态"] = 基本信息.开发动态状态_枚举.开发边界外改造
            elif x["地类编号"][0:2] in ["07", "08", "09", "10", "11", "12", "13", "14", "15", "16"] and x["地类编号"][0:4] not in ["1207", "1202"] and x["所属三线"] == "开发边界内":
                x["开发动态"] = 基本信息.开发动态状态_枚举.开发边界内新建
            elif x["地类编号"][0:2] in ["07", "08", "09", "10", "11", "12", "13", "14", "15", "16"] and x["地类编号"][0:4] not in ["1207", "1202"] and x["所属三线"] != "开发边界内":
                x["开发动态"] = 基本信息.开发动态状态_枚举.开发边界外新建
            else:
                x["开发动态"] = None
...
def 计算地块内设施规模(输入要素名称="DIST_用地规划图", 设施要素名称="SS_配套设施", 输出要素名称="内存临时"):
    ...
    输出要素名称 = 工具包.临时路径生成(["计算地块内设施规模"]) if 输出要素名称 == "内存临时" else 输出要素名称
    ...
    with 游标类.游标创建("更新", 用地要素, 用地游标操作字段) as 用地游标, 游标类.游标创建("查询", 设施要素, 设施游标操作字段) as 设施游标:
...
def 计算地块其他属性(输入要素路径="DIST_用地规划图", 输出要素路径="内存临时"):
    输出要素路径 = 工具包.临时路径生成(["融合部分地类"]) if 输出要素路径 == "内存临时" else 输出要素路径
...
def 计算开口方位(输入要素路径="DIST_用地规划图", 开口符号要素路径="YD_TZ_机动车开口方位", 输出要素路径="内存临时"):
    输出要素路径 = 工具包.输出路径生成_当采用内存临时时(["计算开口方位"]) if 输出要素路径 == "内存临时" else 输出要素路径

    输入要素 = 要素类.要素创建_通过复制(输入要素路径)
    开口符号要素 = 要素类.要素创建_通过复制(开口符号要素路径)

    要素类.字段添加_字符串(开口符号要素, "开口方位")

    开口符号ID列表 = []
    开口符号需操作的字段名称列表 = ["_ID", "Angle", "开口方位"]
    with 游标类.游标创建("更新", 开口符号要素, 开口符号需操作的字段名称列表) as 游标_开口符号:
        for 开口符号x in 游标类.属性获取_数据_字典形式(游标_开口符号, 开口符号需操作的字段名称列表):
            if 45 <= float(开口符号x["Angle"]) < 135:
                开口符号x["开口方位"] = "S"
            elif 135 <= float(开口符号x["Angle"]) < 225:
                开口符号x["开口方位"] = "E"
            elif 225 <= float(开口符号x["Angle"]) < 315:
                开口符号x["开口方位"] = "N"
            elif 315 <= float(开口符号x["Angle"]) <= 360:
                开口符号x["开口方位"] = "W"
            elif 0 <= float(开口符号x["Angle"]) < 45:
                开口符号x["开口方位"] = "W"
            else:
                raise Exception(f'获取到的开口符号的角度不对，{开口符号x["Angle"]}')

            开口符号ID列表.append(开口符号x["_ID"])
            游标类.行更新_字典形式(游标_开口符号, 开口符号x, 开口符号需操作的字段名称列表, 输入操作要素进行数据验证=开口符号要素)

    from bxarcpy.几何包 import 几何类
    from bxpy.基本对象包 import 表类

    用地需操作的字段名称列表 = ["_ID", "_形状", "开口方位"]
    开口符号需操作的字段名称列表 = ["_ID", "_形状", "开口方位"]

    with 游标类.游标创建("更新", 输入要素, 用地需操作的字段名称列表) as 游标_用地, 游标类.游标创建("查询", 开口符号要素, 开口符号需操作的字段名称列表) as 游标_开口符号:
        for 地块x in 游标类.属性获取_数据_字典形式(游标_用地, 用地需操作的字段名称列表):
            当前开口集合 = set()

            for 开口符号x in 游标类.属性获取_数据_字典形式(游标_开口符号, 开口符号需操作的字段名称列表):
                if 几何类.关系_包含(地块x["_形状"], 开口符号x["_形状"]):
                    当前开口集合.add(开口符号x["开口方位"])
                    开口符号ID列表.remove(开口符号x["_ID"])

            游标类.重置(游标_开口符号)
            当前开口列表 = 表类.排序(lambda x: ["E", "S", "W", "N"].index(x), list(当前开口集合))
            当前开口 = "/".join(当前开口列表)

            if 地块x["开口方位"] != 当前开口:
                输入输出类.输出消息(f'ID为{地块x["_ID"]}的地块的开口符号由{地块x["开口方位"]}修改为{当前开口}')
                地块x["开口方位"] = 当前开口

            游标类.行更新_字典形式(游标_用地, 地块x, 用地需操作的字段名称列表, 输入操作要素进行数据验证=输入要素)

    输入输出类.输出消息(f"ID为{开口符号ID列表}的开口符号没有找到对应的地块")
...
def 计算地块其他属性_单个_自生成属性设置(地块对象字典, 临时字典):
    ...
    if 地块对象字典["实体GUID"] in ["", " ", None]:
        输入输出类.输出消息(f"GUID为空：{地块对象字典['_ID']}，GUID：{地块对象字典['实体GUID']}")
        地块对象字典["实体GUID"] = 字类.字符串生成_GUID()
    if len(地块对象字典["实体GUID"].split("-")) == 6:
        地块对象字典["实体GUID"] = "-".join(地块对象字典["实体GUID"].split("-")[0:5])
    if len(地块对象字典["实体GUID"].split("-")) != 5:
        输入输出类.输出消息(f"GUID错误：{地块对象字典['_ID']}，GUID：{地块对象字典['实体GUID']}")
        地块对象字典["实体GUID"] = 字类.字符串生成_GUID()
    ...
    for 属性定义x in 地块属性表:
        if 属性定义x["依托其他字段计算"] is not None:
            ...
            if 地块对象字典[属性定义x["属性名称"]] != 计算结果:
                输入输出类.输出消息(f'GUID为{地块对象字典["实体GUID"]}的项的{属性定义x["属性名称"]}由{地块对象字典[属性定义x["属性名称"]]}修改为{计算结果}')
...
def _公式计算(公式字符串, 变量字典):
    try:
        ...
        raise Exception(f"公式计算错误，公式字符串为：{公式字符串}，变量字典为：{变量字典}, 错误详情为：{追踪元数据类.追踪信息获取()}")
...
if __name__ == "__main__":
    ...
    with 环境管理器类.环境管理器类创建(工作空间):
        # 计算开口方位("DIST_用地规划图", "YD_TZ_机动车开口方位", "DIST_用地规划图1")
        用地更新(
            用地要素路径="DIST_用地规划图",
            局部更新SQL语句="",
            街坊范围线要素名称="JX_街坊范围线",
            ...
            开口符号要素路径="YD_TZ_机动车开口方位",
            是否输出更新前后变化日志=True,
            日志输出路径="C:\\Users\\beixiao\\Desktop\\debug_用地更新.txt",
            输出要素路径="DIST_用地规划图1",
        )

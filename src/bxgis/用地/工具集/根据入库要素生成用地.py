...
def 用地_工具集_根据入库要素生成用地(
    ...
    日志生成器.临时关闭日志()
    # 需要提前完成编号、耕地面积计算
    输出要素名称 = 工具包.临时路径生成(["根据入库要素生成用地"]) if 输出要素名称 == "内存临时" else 输出要素名称
    # if 输出要素名称 == "内存临时":
    ...
    for 字段名称x in 字段名称列表:
        要素类.字段修改(地块要素, 字段名称x, 字段名称x.upper())
    ...
    操作字段名称列表 = ["DLDM", 地类编号字段名称, "FL", 用地大类字段名称, "GHDT", 开发动态字段名称, "FJSS1", "FJSS2", 配套设施规模字段名称]
    with 游标类.游标创建("更新", 地块要素, 操作字段名称列表) as 游标:
        for 游标数据x in 游标类.属性获取_数据_字典形式(游标, 操作字段名称列表):
            if 游标数据x["FL"] != "村庄建设用地":
                游标数据x[地类编号字段名称] = 游标数据x["DLDM"]
            elif len(游标数据x["DLDM"]) >= 4 and 游标数据x["DLDM"][0:4] in ["0703", "0704"]:
                游标数据x[地类编号字段名称] = 游标数据x["DLDM"]
            else:
                if "/" in 游标数据x["DLDM"]:
            ...
            附建设施1_TEMP = "" if 游标数据x["FJSS1"] in ["", None, " "] else 游标数据x["FJSS1"]
            附建设施2_TEMP = "" if 游标数据x["FJSS2"] in ["", None, " "] else 游标数据x["FJSS2"]
            游标数据x[配套设施规模字段名称] = str(附建设施1_TEMP) + str(附建设施2_TEMP)
...
if __name__ == "__main__":
    ...
    # 工作空间 = r"C:\Users\Common\Project\H杭州配套设施优化研究\H杭州配套设施优化研究_数据库.gdb"

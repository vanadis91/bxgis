...
def 用地规划图生成(
    ...
    # 合并各图层
    底层要素 = bxarcpy.要素类.要素读取_通过名称(输入要素名称列表[0]).要素创建_通过复制().字段删除(保留字段名称列表=[配置.地块要素字段映射.地类编号字段名称])
    for x in 输入要素名称列表[1:]:
        底层要素 = 底层要素.要素创建_通过更新(x).字段删除(保留字段名称列表=[配置.地块要素字段映射.地类编号字段名称])
        时间.等待(1)
        bxarcpy.环境.输出消息(f"完成了 {x} 的合并")
    if 规划范围线要素名称:
        底层要素 = 底层要素.要素创建_通过裁剪(规划范围线要素名称)
    ...
    现有的CAD色块外建设用地修改元素 = bxarcpy.要素类.要素读取_通过名称(GIS中更改以CAD为准地类色块的要素名称).要素创建_通过复制()
    ...
    GIS中更改以CAD为准地类色块的要素 = 更新后要素.要素创建_通过融合([配置.地块要素字段映射.地类编号字段名称]).要素创建_通过复制并重命名重名要素(GIS中更改以CAD为准地类色块的要素名称)

    判断是否需要处理GIS中更改以CAD为准地类色块的要素 = GIS中更改以CAD为准地类色块的要素.要素创建_通过筛选(SQL_以CAD为准的地类)
    if 判断是否需要处理GIS中更改以CAD为准地类色块的要素.几何数量 > 0:
        日志.输出警告(f"GIS中更改以CAD为准地类色块的要素 中 存在部分 CAD色块 才拥有的地类")
    else:
        日志.输出调试(f"GIS中更改以CAD为准地类色块的要素 中 无 CAD色块 才拥有的地类")

    DIST_用地规划图1 = DIST_用地规划图1.要素创建_通过更新(GIS中更改以CAD为准地类色块的要素.名称)

    # 处理细小面
    细小面要素 = DIST_用地规划图1.要素创建_通过筛选("Shape_Area <= " + 细小面面积阈值).字段删除(["所属地块"])
    if GIS中已处理的细小面要素名称:
        GIS中已处理的细小面要素 = bxarcpy.要素类.要素读取_通过名称(GIS中已处理的细小面要素名称).要素创建_通过复制()
        GIS中已处理的细小面要素.要素创建_通过空间连接(DIST_用地规划图1.名称, "大部分在连接要素内").字段添加("所属地块", 字段类型="长整型").字段计算("所属地块", f"int(!FID_{DIST_用地规划图1.名称_无路径}!)").字段删除(保留字段名称列表=["所属地块"])
        细小面要素.要素创建_通过空间连接(GIS中已处理的细小面要素.名称, "在连接要素内")
    else:
        细小面要素.字段添加("所属地块", "长整型")
    ...
    DIST_用地规划图1 = DIST_用地规划图1.要素创建_通过更新并合并字段(更新后已处理细小面要素.名称)

    ## 细小面要素改名为YD_已处理细小面
    更新后已处理细小面要素.要素创建_通过复制并重命名重名要素('YD_已处理的细小面')

    ## 从细小面要素与分离出YD_未处理细小面
    未处理细小面要素 = 细小面要素.要素创建_通过筛选("'所属地块 IS NULL'").字段删除(['所属地块'])
    if 未处理细小面要素.几何数量 > 0:
        日志.输出警告(f"当前存在部分细小面需要明确是否保留，如需保留请将细小面与周边面合并后，复制至YD_已处理的细小面。")
    else:
        日志.输出调试(f"当前不存在需要明确的细小面。")
        

    # 删除规划范围以外区域
    if 规划范围线要素名称:
        DIST_用地规划图1 = DIST_用地规划图1.要素创建_通过裁剪(规划范围线要素名称)

    # 最终导出
    输出要素 = DIST_用地规划图1.要素创建_通过多部件至单部件().字段删除(["综合耕地N", "处理方向", "综合净面积", "综合耕地N_1", "处理方向_1", "综合净面积_1", "ORIG_FID"]).要素创建_通过复制并重命名重名要素(输出要素名称)
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        用地规划图生成(
            输入要素名称列表=["YD_基期细化", "YD_农转用20年及以前", "YD_现状修改1", "YD_农转用21年及以后", "YD_审批信息已实施", "YD_地籍信息", "YD_现状修改2", "YD_审批信息已批未建", "YD_上位_粮食生产功能区", "YD_上位基本农田落实", "YD_上位农用地落实_耕地质量提升", "YD_上位农用地落实_旱改水", "YD_上位农用地落实_垦造耕地", "YD_上位农用地落实_新增设施农用地", "YD_GIS方案_农用地设计"],
            CAD导出色块要素名称="YD_CAD色块",
            GIS中更改以CAD为准地类色块的要素名称="YD_CAD色块以外建设用地修改",
            CAD导出色块中空隙的地类="'00'",
            CAD导出色块中有效的地类列表="地类编号 LIKE '07%' OR 地类编号 LIKE '08%' OR 地类编号 LIKE '09%' OR 地类编号 LIKE '10%' OR 地类编号 LIKE '11%'  OR 地类编号 LIKE '12%'  OR 地类编号 LIKE '13%'  OR 地类编号 LIKE '14%'  OR 地类编号 LIKE '15%'  OR 地类编号 LIKE '16%'  OR ( 地类编号 LIKE '17%' AND 地类编号 NOT LIKE '1704%' AND 地类编号 NOT LIKE '1705%' )  OR 地类编号 LIKE '2301%'",
            规划范围线要素名称="JX_规划范围线",
            输出要素名称="DIST_用地规划图",
            是否拓扑检查=False,
            是否范围检查=False,

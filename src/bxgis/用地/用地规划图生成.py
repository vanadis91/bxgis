...
def 用地规划图生成(
    ...
    if 是否将CAD合并入GIS:
        ...
        if 判断是否需要处理GIS中更改以CAD为准地类色块的要素.几何数量 > 0:
            日志.输出警告(f"CAD导出色块以外地类调整要素 中 存在部分 CAD色块 才拥有的地类")
        else:
            日志.输出调试(f"CAD导出色块以外地类调整要素 中 无 CAD色块 才拥有的地类")
    ...
    if 是否处理细小面:
        ...
        if 未处理细小面要素.几何数量 > 0:
            日志.输出警告(f"当前存在部分细小面需要明确是否保留，如需保留请将细小面与周边面合并后，复制至{GIS中已处理的细小面要素名称}。")
        else:

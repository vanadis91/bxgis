...
def _处理CAD方案外建设用地(
    ...
    if 要素类.属性获取_几何数量(判断是否需要处理GIS中更改以CAD为准地类色块的要素) > 0:
        ...
        日志类.输出调试(f"CAD导出色块以外区域的地类都微调完毕了")
...
def _细小面处理(
    ...
    if 要素类.属性获取_几何数量(筛选那些还需要处理的细小面) > 0:
        日志类.输出警告(f"尚有部分的细小面需要进行处理")
    else:
        日志类.输出调试(f"所有的细小面都处理完毕了")
    ...
    if 要素类.属性获取_几何数量(用于验证细小面要素) > 0:
        日志类.输出调试(f"细小面合并后仍有面积较小的地块，已导出到【AA_细小面合并后面积仍较小的地块】")
...
if __name__ == "__main__":
    ...
    临时工作空间 = r"C:\Users\common\project\J江东区临江控规\临江控规_数据库_临时.gdb"
    with 环境管理器类.环境管理器类创建(工作空间, 临时工作空间):
        用地规划图生成(
            输入要素名称列表=[
                "YD_基期细化",
                "YD_农转用20年及以前",

...
def 开发边界外城镇建设用地检查(
    ...
    if 日志类.属性获取_当前函数内日志开启状态():
        ...
        日志类.输出调试并暂停(f"已输出AA_边界外建设用地")

    边界内非建设用地 = 要素类.要素创建_通过筛选(边界内用地, "地类编号 LIKE '01%' OR 地类编号 LIKE '02%' OR 地类编号 LIKE '03%' OR 地类编号 LIKE '04%' OR 地类编号 LIKE '05%' OR 地类编号 LIKE '06%' OR 地类编号 LIKE '0703%' OR 地类编号 LIKE '0704%' OR 地类编号 LIKE '%v' OR (地类编号 LIKE '17%' AND 地类编号 NOT LIKE '1701%')")
    if 日志类.属性获取_当前函数内日志开启状态():
        要素类.要素创建_通过复制并重命名重名要素(边界内非建设用地, "AA_边界内非建设用地")
        日志类.输出调试并暂停(f"已输出AA_边界内非建设用地")

    边界外建设用地 = 要素类.要素创建_通过多部件至单部件(边界外建设用地)
    边界内非建设用地 = 要素类.要素创建_通过多部件至单部件(边界内非建设用地)

    if 输出要素名称_开发边界内 == "内存临时":
        输出要素名称_开发边界内 = "in_memory\\AA_边界内用地" + "_" + 工具包.生成短GUID()
    if 输出要素名称_开发边界外 == "内存临时":
        输出要素名称_开发边界外 = "in_memory\\AA_边界外用地" + "_" + 工具包.生成短GUID()
    if 输出要素名称_开发边界外建设用地 == "内存临时":
        输出要素名称_开发边界外建设用地 = "in_memory\\AA_边界外建设用地" + "_" + 工具包.生成短GUID()
    if 输出要素名称_开发边界内非建设用地 == "内存临时":
        输出要素名称_开发边界内非建设用地 = "in_memory\\AA_边界内非建设用地" + "_" + 工具包.生成短GUID()

...
def 设施更新(
    ...
    输出要素路径_用地 = 工具包.临时路径生成([输入要素路径]) if 输出要素路径_用地 == "内存临时" else 输出要素路径_用地
    输出要素路径_设施 = 工具包.临时路径生成([输入要素路径]) if 输出要素路径_设施 == "内存临时" else 输出要素路径_设施

    设施要素路径 = 要素类.要素创建_通过复制(输入要素路径)
    设施要素路径_修改前 = 要素类.要素创建_通过复制(输入要素路径)
    用地规划要素路径 = 要素类.要素创建_通过复制(用地规划要素路径)
    用地规划要素路径_修改前 = 要素类.要素创建_通过复制(用地规划要素路径)
    ...
    字段名称列表 = 要素类.字段名称列表获取(设施要素路径_修改前, 含系统字段=False)
    字段名称列表 = [[x, x] for x in 字段名称列表]
    属性对比(设施要素路径_修改前, 输出要素_设施, [基本信息.设施要素字段映射.实体GUID字段名称, 基本信息.设施要素字段映射.实体GUID字段名称], 对比字段名称列表=字段名称列表)

    字段名称列表 = 要素类.字段名称列表获取(用地规划要素路径_修改前, 含系统字段=False)
    字段名称列表 = [[x, x] for x in 字段名称列表]
    属性对比(用地规划要素路径_修改前, 输出要素_用地, [基本信息.地块要素字段映射.实体GUID字段名称, 基本信息.地块要素字段映射.实体GUID字段名称], 对比字段名称列表=字段名称列表)
...
def _计算远期预留(输入要素路径, 城镇集建区要素名称="KZX_城镇集建区", 城镇弹性区要素名称="KZX_城镇弹性区", 输出要素路径="内存临时"):
    ...
    要素类.字段添加(设施要素, "所属三线")
    设施要素 = 属性更新.常用_属性更新(设施要素, 开发边界要素, 字段映射列表=[["所属三线", 基本信息.控制线要素字段映射.控制线名称字段名称]], 计算方式="内点在区域要素内")
...
if __name__ == "__main__":
    ...
    日志处理器.输出器_文件对象_路径 = r"C:\Users\beixiao\Desktop\debug_设施更新.txt"
    日志生成器.开启()

...
def _和地块中配套信息进行统一(输入要素路径, 用地要素路径, 同步方式: Literal["从设施到用地", "从用地到设施"] = "从用地到设施", 输出要素路径="内存临时"):
    输出要素路径 = 输出路径生成_当采用内存临时时([输入要素路径]) if 输出要素路径 == "内存临时" else 输出要素路径

    设施要素 = 要素类.要素创建_通过复制(输入要素路径)
    用地要素 = 要素类.要素创建_通过复制(用地要素路径)

    用地操作字段名称列表 = [基本信息.地块要素字段映射.配套设施规模字段名称, "_形状"]
    设施操作字段名称列表 = [
        基本信息.设施要素字段映射.设施名称字段名称,
        "_形状",
        基本信息.设施要素字段映射.设施数量字段名称,
        基本信息.设施要素字段映射.设施规模字段名称,
    ]
    with 游标类.游标创建("更新", 用地要素, 用地操作字段名称列表) as 游标_用地:
        with 游标类.游标创建("更新", 设施要素, 设施操作字段名称列表) as 游标_设施:
            for 用地x in 游标类.属性获取_数据_字典形式(游标_用地, 用地操作字段名称列表):
                ...
                for x in 设施列表temp:
                    ...
                    if len(单个设施列表) == 3:  # 存在规模
                        设施规模 = 单个设施列表[2].split(',')
                        设施列表.append({"设施名称": 设施名称, "设施数量": 设施数量, "设施规模": 设施规模})
                    else:
                        设施列表.append({"设施名称": 设施名称, "设施数量": 设施数量})

                for 设施x in 游标类.属性获取_数据_字典形式(游标_设施, 设施操作字段名称列表):
                    from bxarcpy.几何包 import 几何类
                    当前设施 = {"设施名称": 设施x[基本信息.设施要素字段映射.设施名称字段名称], "设施数量": 基本信息.设施要素字段映射.设施数量字段名称, "设施规模": 设施x[基本信息.设施要素字段映射.设施规模字段名称]}
                    
                    if 几何类.关系_包含(用地x["_形状"], 设施x["_形状"]):
                        if 当前设施['设施名称'] in [x['设施名称'] for x in 设施列表]:
                            用地中设施  = [x for x in 设施列表 if x['设施名称'] == 当前设施['设施名称']][0]
                            设施索引 = 设施列表.index(用地中设施)
                            if '设施规模' in 当前设施 and 当前设施['设施规模'] in 用地中设施['设施规模']:
                                设施列表[设施索引]['设施规模'].remove(当前设施['设施规模'])
                            if '设施规模' in 当前设施 and 当前设施['设施规模'] not in 用地中设施['设施规模'] and 同步方式 =='从用地到设施':

...
def 项目初始化(项目文件夹路径=r"C:\Users\beixiao\Desktop"):
    ...
    yml文件路径 = 路径类.连接(项目文件夹路径, ".bxgis", "项目信息.yml")
    if not 路径类.是否存在(yml文件路径):
        项目信息 = {"项目基本信息": {"工作空间": 数据库路径, "项目目录": 项目文件夹路径}}
        路径类.新增文件(yml文件路径)
        字典类.转换_到文件(项目信息, yml文件路径)
        日志生成器.输出控制台(f"未在 项目根目录/.bxgis 找到 项目信息.yml，因此自动创建了该文件")

    当前项目配置 = 字典类.转换_从文件(yml文件路径)

    默认项目信息文件路径 = 路径类.连接(路径类.属性获取_目录(__file__), "项目信息_默认.yml")
    默认配置 = 字典类.转换_从文件(默认项目信息文件路径)

    最终配置 = 字典类.解包合并(默认配置, 当前项目配置)
    日志生成器.输出控制台(f"最终配置为：{字类.转换_到字(最终配置,json缩进=2)}", 内容长度=20000)
    字典类.转换_到文件(最终配置, yml文件路径)
    日志生成器.输出控制台(f"可通过修改 {yml文件路径} 中内容来进一步完善配置，修改后需重新运行项目初始化命令")

    应用配置文件路径 = 路径类.连接(路径类.属性获取_目录(__file__), "应用配置.yml")
    应用配置字典 = 字典类.转换_从文件(应用配置文件路径)
    应用配置字典["应用基本信息"]["工作空间"] = 数据库路径
    应用配置字典["应用基本信息"]["项目目录"] = 项目文件夹路径
    字典类.转换_到文件(应用配置字典, 应用配置文件路径)

...
def 河道边线生成(河道中线要素名称="DL_河道中线", 用地要素名称="DIST_用地规划图", 河道用地修正要素名称: Union[str, None] = "DL_河道用地修正", 规划范围线要素名称="JX_规划范围线", 修正要素路径=None, 输出要素名称="内存临时"):
    ...
    河道中线缓冲 = 要素类.要素创建_通过缓冲(河道中线, "河道半宽", 融合类型="融合按字段", 融合字段名称列表=["河道名称", "开发动态"], 末端类型="方形")
    要素类.字段删除(河道中线缓冲, ["河道半宽"])

    操作字段 = ["_ID", "_形状", "河道名称", "开发动态"]
    ...
    用地中河道 = 要素类.要素创建_通过筛选(地块要素, "地类编号 LIKE '1701%'")
    河道用地 = 要素类.要素创建_通过联合([用地中河道, 河道用地])
    用地中非河道 = 要素类.要素创建_通过擦除(河道用地, 用地中河道)
    要素类.要素创建_通过复制并重命名重名要素(用地中非河道, "DL_河道用地修正")
    ...
    要素类.字段添加(还没有属性的河道边线要素, "开发动态")

    操作字段列表 = ["_形状", "河道名称", "开发动态"]
    with 游标类.游标创建("更新", 还没有属性的河道边线要素, 操作字段列表) as 游标1:
        with 游标类.游标创建("查询", 带有属性的河道边线要素, 操作字段列表) as 游标2:
            for x in 游标类.属性获取_数据_字典形式(游标1, 操作字段列表):
                for y in 游标类.属性获取_数据_字典形式(游标2, 操作字段列表):
    ...
    if 修正要素路径:
        ...
        合并后要素 = 要素类.要素创建_通过联合并赋值字段(合并后要素, 修正要素, [["河道名称", "河道名称"], ["开发动态", "开发动态"]], 是否去除输入无联合有的部分=True, 是否检查两者差异=False, 是否删除字段的既有值=False)

    范围 = 要素类.要素创建_通过复制(规划范围线要素名称)
    范围转线 = 要素类.要素创建_通过转线(范围)
    合并后要素 = 要素类.要素创建_通过擦除_原始(合并后要素, 范围转线)

    合并后要素 = 要素类.要素创建_通过融合(合并后要素, ["河道名称", "开发动态"])
    要素类.字段删除(合并后要素, 保留字段名称列表=["河道名称", "开发动态"])

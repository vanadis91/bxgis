...
def 道路边线生成(道路中线要素路径="DL_道路中线", 用地要素名称="DIST_用地规划图", 规划范围线要素名称="JX_规划范围线", 输出要素路径="内存临时"):
    if 输出要素路径 == "内存临时":
    ...
    with 游标类.游标创建("更新", 道路中线缓冲要素, 操作字段名称列表) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, 操作字段名称列表):
            if x["道路等级"] == "城市次干路":
                x["道路等级映射"] = 3.0
            elif x["道路等级"] == "城市快速路":
                x["道路等级映射"] = 1.0
    ...
    with 游标类.游标创建("更新", 道路中线缓冲要素, 操作字段名称列表) as 游标1:
        with 游标类.游标创建("查询", 道路中线缓冲要素, 操作字段名称列表) as 游标2:
            # 游标1运行次数 = 0
            for x in 游标类.游标迭代器转换_字典形式(游标1, 操作字段名称列表):
                # 游标1运行次数 += 1
                # 游标2运行次数 = 0
                # if x["道路等级"] == "高速公路":
                #     游标1.行删除()
                #     break
                for y in 游标类.游标迭代器转换_字典形式(游标2, 操作字段名称列表):
                    # 游标2运行次数 += 1
    ...
    带有属性的道路边线要素 = 要素类.要素创建_通过相交([道路边线要素, 道路中线缓冲要素])
    带有属性的道路边线要素 = 要素类.要素创建_通过多部件至单部件(带有属性的道路边线要素)
    带有属性的道路边线要素 = 要素类.要素几何修复(带有属性的道路边线要素)
    # 带有属性的道路边线要素.要素创建_通过复制并重命名重名要素("AA_A带有属性的那些")
    还没有属性的道路边线要素 = 要素类.要素创建_通过擦除(道路边线要素, 道路中线缓冲要素)
    还没有属性的道路边线要素 = 要素类.要素创建_通过多部件至单部件(还没有属性的道路边线要素)
    还没有属性的道路边线要素 = 要素类.要素几何修复(还没有属性的道路边线要素)

...
def 道路边线生成(
    ...
    道路中线缓冲要素 = 要素类.要素创建_通过缓冲(道路中线要素路径_复制后, "道路半宽", 融合类型="融合按字段", 融合字段名称列表=["道路等级", "道路名称", "开发动态"], 末端类型="方形")
    ...
    操作字段名称列表 = ["_ID", "_形状", "道路等级", "道路名称", "开发动态"]
    ...
    if 日志生成器.属性获取_当前函数内日志开启状态():
        要素类.要素创建_通过复制并重命名重名要素(带有属性的道路边线要素, "AA_带有属性的道路边线要素")
        日志生成器.输出调试并暂停(f"输出了带有属性的道路边线要素")
    ...
    合并后要素 = 要素类.要素创建_通过融合(合并后要素, ["道路名称", "道路等级", "开发动态"])
    要素类.字段删除(合并后要素, 保留字段名称列表=["道路名称", "道路等级", "开发动态"])
    # 修正道路边线

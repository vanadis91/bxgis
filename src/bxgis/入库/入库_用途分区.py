...
_用途分区要求 = {
    ...
    "林业发展区": "林业发展区内的林地根据林地种类按照《林地保护等级及保护管理措施》III级林地、IV级林地的要求进行管控。除生态保护区和生态控制区之外的林地集中分布区域，主要将人工商品林地、树圃、苗圃、经济林集中区域划为该区。分区界线应按照结合国土调查林地图斑边界和林地小斑综合确定。林业发展区内林地面积占比应达到70%以上。",
}
_用途分区代码映射 = [
...
def 入库_用途分区(分区要素名称="DIST_用途分区图", 单元名称="临江单元", 批复时间="", 批复文号="", 单元编号="QT12", 输出要素名称="XG_YTFQ"):
    ...
    日志生成器.输出调试(f"用途分区要素字段列表：{要素类.字段名称列表获取(用途分区要素)}")

    分区名称字段名称 = 基本信息.分区要素字段映射.分区名称字段名称
    分区编号字段名称 = 基本信息.分区要素字段映射.分区编号字段名称
    分区要求字段名称 = 基本信息.分区要素字段映射.分区要求字段名称
    用途分区要素 = 要素类.要素创建_通过融合(用途分区要素, [分区名称字段名称, 分区要求字段名称], 是否计算面积字段=True)
    ...
    要素类.字段计算(用途分区要素, "FQYQ", f"!{分区要求字段名称}!")
    ...
    if "XG_YTFQ" in 数据库类.属性获取_要素名称列表(数据库 + "/XG"):
        from bxgis.属性.属性对比 import 属性对比

        字段名称列表 = 要素类.字段名称列表获取("XG/XG_YTFQ", 含系统字段=False)
    ...
    输出要素 = 要素类.要素创建_通过复制并重命名重名要素(用途分区要素, f"XG/" + 输出要素名称)
    return 输出要素

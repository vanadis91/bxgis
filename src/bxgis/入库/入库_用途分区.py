...
def 入库_用途分区(分区要素名称="DIST_用途分区图", 单元名称="临江单元", 批复时间="", 批复文号="", 单元编号="QT12", 分区名称字段名称="用途管制分区", 分区编号字段名称="用途分区编号", 输出要素名称="XG_YTFQ"):
    用途分区要素 = bxarcpy.要素类.要素读取_通过名称(分区要素名称).要素创建_通过复制()
    ...
    用途分区要素 = 用途分区要素.要素创建_通过融合([分区名称字段名称]).要素创建_通过多部件至单部件()
    用途分区要素.字段添加(分区编号字段名称)
    用途分区要素 = 用途分区要素.要素创建_通过排序([[分区名称字段名称, "正序"]])
    with bxarcpy.游标类.游标创建_通过名称("更新", 用途分区要素.名称, [分区名称字段名称, 分区编号字段名称]) as 游标:
        用途分区编号 = 0
        for x in 游标:
            用途分区编号 += 1
            x[分区编号字段名称] = 单元编号 + "-" + "YTFQ" + str(用途分区编号).zfill(2)
            游标.行更新(x)
    用途分区要素.字段添加("FQBM", "字符串", 20, "用途分区编码").字段计算("FQBM", f"!{分区编号字段名称}!")

    用途分区要素.字段添加("FQDM", "字符串", 255, "用途分区代码")
    with bxarcpy.游标类.游标创建_通过名称("更新", 用途分区要素.名称, [分区名称字段名称, "FQDM"]) as 游标:
    ...
    用途分区要素.字段添加("FQMC", "字符串", 255, "用途分区名称").字段计算("FQMC", f"!{分区名称字段名称}!")
    用途分区要素.字段添加("FQMJ", "双精度", 50, "用途分区面积").字段计算("FQMJ", "round(!Shape_Area!/10000, 4)")

    用途分区要素.字段添加("FQYQ", "字符串", 255, "用途分区要求")
    with bxarcpy.游标类.游标创建_通过名称("更新", 用途分区要素.名称, [分区名称字段名称, "FQYQ"]) as 游标:
        for x in 游标:
    ...
    输出要素 = 用途分区要素.要素创建_通过复制并重命名重名要素(f"入库材料/" + 输出要素名称)
    return 输出要素
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        入库_用途分区(分区要素名称="DIST_用途分区图", 单元名称="临江单元", 批复时间="", 批复文号="", 单元编号="QT12")

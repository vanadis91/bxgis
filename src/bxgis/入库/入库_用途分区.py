...
def 入库_用途分区(分区要素名称="DIST_用途分区图", 单元名称="临江单元", 批复时间="", 批复文号="", 单元编号="QT12", 分区名称字段名称="用途管制分区", 分区编号字段名称="用途分区编号", 输出要素名称="XG_YTFQ"):
    ...
    用途分区要素 = 要素类.要素创建_通过融合(用途分区要素, [分区名称字段名称])
    ...
    with 游标类.游标创建("更新", 用途分区要素, [分区名称字段名称, 分区编号字段名称]) as 游标:
        用途分区编号 = 0
        for x in 游标类.游标迭代器转换_字典形式(游标, [分区名称字段名称, 分区编号字段名称]):
            用途分区编号 += 1
    ...
    with 游标类.游标创建("更新", 用途分区要素, [分区名称字段名称, "FQDM"]) as 游标:
        ...
        for x in 游标类.游标迭代器转换_字典形式(游标, [分区名称字段名称, "FQDM"]):
            映射列表 = [y for y in _用途分区代码映射 if y["用途分区名称"] == x[分区名称字段名称]]
            if len(映射列表) > 0:
                x["FQDM"] = 映射列表[0]["用途分区代码"]
                游标类.行更新_字典形式(游标, x)
            else:
                输入输出类.输出消息(f"未找到用途分区名称 {x['用途管制分区']} 对应的用途分区代码")
    ...
    with 游标类.游标创建("更新", 用途分区要素, [分区名称字段名称, "FQYQ"]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, [分区名称字段名称, "FQYQ"]):

...
def 入库_村庄建设边界(村庄建设边界要素名称="KZX_村庄建设边界", 单元名称="临江单元", 批复时间="", 批复文号="", 单元编号="QT12", 输出要素名称="XG_JSBJ"):
    村庄建设边界要素 = 要素类.要素创建_通过复制(村庄建设边界要素名称)
    村庄建设边界要素 = 要素类.要素创建_通过融合(村庄建设边界要素, None)
    ...
    with 游标类.游标创建("更新", 村庄建设边界要素, ["JSBM"]) as 游标:
        编号 = 0
        for x in 游标类.游标迭代器转换_字典形式(游标, ["JSBM"]):
            编号 += 1
    ...
    with 游标类.游标创建("更新", 村庄建设边界要素, ["GKYQ"]) as 游标:
        for x in 游标类.游标迭代器转换_字典形式(游标, ["GKYQ"]):
    ...
    输出要素 = 要素类.要素创建_通过复制并重命名重名要素(村庄建设边界要素, f"入库材料" + "/" + 输出要素名称)
    return 输出要素

...
def 入库_规划地块(
    ...
    要素类.字段计算(地块要素, "GDMJ", "round(float(!耕地保有量!), 2)")
    ...
    需操作的字段名称列表 = ["XGLX", "绝对高度", "配套设施规模", "FJSS1", "FJSS2", "开发动态", "GHDT", "地块编号", "TDM", "地类编号", "所属街区", "所属街坊", "BZ", "备注说明", "兼容比例", "JRBL", "RJL", "容积率", "LDL", "绿地率", "JZMD", "建筑密度", "JZGD", "建筑限高", "FL", "用地构成", "住宅建筑面积"]

    with 游标类.游标创建("更新", 地块要素, 需操作的字段名称列表) as 游标:
        for x in 游标类.属性获取_数据_字典形式(游标, 需操作的字段名称列表):
            x["BZ"] = None if x["备注说明"] in ["", " ", None] else x["备注说明"]
            if x["绝对高度"] in ["51.7", "65", "156.7"]:
                x["XGLX"] = "机场限高"
                if x["BZ"] is not None:
                    x["BZ"] = x["BZ"] + f'，限高{x["绝对高度"]}米（1985国家高程基准）'
                else:
            ...
            if 是否将住宅兼容比例转化为备注栏中规模管控:
                if x["JRBL"] not in ["", " ", None] and float(x["住宅建筑面积"]) > 0:
                    x["JRBL"] = None
                    住宅建筑规模_万方 = float(x["住宅建筑面积"]) * 0.0001
                    if x["BZ"] is not None:
                        x["BZ"] = x["BZ"] + f"，住宅计容建筑面积不大于{住宅建筑规模_万方:.2f}万方"
                    else:
                        x["BZ"] = f"住宅计容建筑面积不大于{住宅建筑规模_万方:.2f}万方"

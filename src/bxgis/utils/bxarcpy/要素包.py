...
class 要素类:
        ...
        if 字段映射列表:
            ...
            for 字段映射元祖 in 字段映射列表:
                ...
                if 字段映射元祖[0] not in 输入要素字段名称列表:
                    ...
                    要素类.字段添加(输入要素路径, 字段映射元祖[0], 字段类型, 字段长度)  # type: ignore
    ...
    def 要素创建_通过空间连接(输入要素路径, 连接要素路径, 连接方式: Literal["相交", "包含连接要素", "完全包含连接要素", "在连接要素内", "完全在连接要素内", "包含连接要素内点", "内点在连接要素内", "形心在连接要素内", "大部分在连接要素内"] = "包含连接要素", 输出要素路径="内存临时"):
        ...
        if 连接方式raw == "内点在连接要素内":
            ...
            目标要素路径 = 要素类.要素创建_通过复制并重命名重名要素(输入要素路径, "AA_准备转点")
            ...
            # input(要素类.字段名称列表获取(目标要素路径_转点后_带连接要素字段, False))
            保留字段名称列表raw = 要素类.字段名称列表获取(连接要素路径, False)
            目标要素名称 = 要素类.属性获取_要素名称(目标要素路径)

            FID字段名称 = "FID_" + 目标要素名称
            目标要素路径_转点后_带连接要素字段_字段名称列表 = 要素类.字段名称列表获取(目标要素路径_转点后_带连接要素字段, False)
            if FID字段名称 in 目标要素路径_转点后_带连接要素字段_字段名称列表:
                FID字段名称 = FID字段名称
            else:
                raise Exception(f"没有找到目标要素转点前的ID字段名称，需要查找的字段为：{FID字段名称}，字段列表为：{目标要素路径_转点后_带连接要素字段_字段名称列表}")
            ...
            # input(目标要素转点后操作字段列表)
            with 游标类.游标创建("更新", 目标要素路径, 目标要素操作字段列表) as 面要素游标对象, 游标类.游标创建("查询", 目标要素路径_转点后_带连接要素字段, 目标要素转点后操作字段列表) as 点要素游标对象:  # type: ignore
    ...
    def 要素创建_通过转点(输入要素路径, 输出要素路径="内存临时"):
        ...
        # print(点字段列表)
        with 游标类.游标创建("查询", 输入要素路径, 字段列表) as 面要素游标对象, 游标类.游标创建("插入", 点要素路径, 点字段列表) as 点要素游标对象:
    ...
    def 字段列表获取(输入要素路径, 含系统字段=True):
        if 含系统字段:
            ...
            字段列表 = [x for x in arcpy.ListFields(输入要素路径)]  # type: ignore
            字段列表 = [x for x in 字段列表 if 字段类.属性获取_名称(x).upper() not in ["OBJECTID", "OBJECTID_1", "SHAPE", "SHAPE_1", "SHAPE_LENGTH", "SHAPE_LENGTH_1", "SHAPE_AREA", "SHAPE_AREA_1", "OID", "OID_1"]]
        return 字段列表
    ...
    def 字段添加_通过字段对象(输入要素路径, 字段对象):
        ...
        要素类.字段添加(输入要素路径, 字段名称=字段名称, 字段类型=字段类型, 字段长度=字段长度, 字段别称=字段别称)  # type: ignore
        return 输入要素路径
        ...
        if 字段放置于尾部:
            ...
            for 字段名称 in 字段名称顺序列表:
                ...
                要素类.字段添加(
                    输入要素路径,
                    随机新字段名称,
                    字段类型=字段类.属性获取_类型(字段对象),  # type: ignore
                    字段长度=字段类.属性获取_长度(字段对象),
                )
...
class 项类:
    ...
    def 项更新_通过面(输入要素路径, 面要素名称="JX_街区范围线", 字段映射列表=[["所属街区", "街区编号"]], 计算方式: Literal["分割输入要素", "内点在面要素内", "大部分在面要素内"] = "分割输入要素", 输出要素路径="内存临时"):
        输出要素路径 = 工具包.临时路径生成(["项更新_通过面"]) if 输出要素路径 == "内存临时" else 输出要素路径
    ...
    def 项更新_通过点(输入要素路径, 点要素名称="AA_test", 字段映射列表=[["所属街区", "街区编号"]], 输出要素路径="内存临时"):
        输出要素路径 = 工具包.临时路径生成(["项更新_通过点"]) if 输出要素路径 == "内存临时" else 输出要素路径
...
if __name__ == "__main__":
    ...
    with 环境管理器类.环境管理器类创建(工作空间):
        ...
        要素类.要素创建_通过属性域修复("JX_规划范围线", "JX_规划范围线1")

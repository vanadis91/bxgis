...
class 参数类:
    ...
    @staticmethod
    def 属性获取_参数名称(参数对象: arcpy.Parameter):
        return 参数对象.name

    @staticmethod
    def 属性设置_是否可用(参数对象: arcpy.Parameter, 布尔值):
    ...
    @staticmethod
    def 参数列表转换_到字典_参数对象转换为值(参数列表):
        from bxarcpy.参数包 import 参数类

        参数名称列表 = [参数类.属性获取_参数名称(x) for x in 参数列表]
        参数字典 = {k: v for k, v in zip(参数名称列表, 参数列表)}
        参数字典temp = {}

        def 根据类型取值(参数对象):
            if type(参数类.属性获取_值(参数对象)) in [int, float, str, bool]:
                ret = 参数类.属性获取_值(参数对象)
            elif type(参数类.属性获取_值(参数对象)) is list:
                ret = [根据类型取值(x) for x in 参数对象]
            else:
                ret = 参数类.属性获取_值_字符串形式(参数对象)
            return ret

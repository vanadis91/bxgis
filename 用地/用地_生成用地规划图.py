...
def 用地创建_生成用地规划图(
    ...
    for x in 输入要素名称列表[1:]:
        底层要素 = 底层要素.要素创建_通过更新(x).字段删除(保留字段名称列表=["地类编号"])

        时间.等待(1)
        日志.输出控制台(f"完成了 {x} 的合并")
    底层要素 = 底层要素.要素创建_通过裁剪(范围要素名称)
    ...
    GIS中更改以CAD为准地类色块的要素 = 更新后要素.要素创建_通过融合(["地类编号"]).要素创建_通过复制并重命名重名要素(GIS中更改以CAD为准地类色块的要素名称)

    判断是否需要处理GIS中更改以CAD为准地类色块的要素 = GIS中更改以CAD为准地类色块的要素.要素创建_通过筛选(SQL_以CAD为准的地类)
    if 判断是否需要处理GIS中更改以CAD为准地类色块的要素.几何数量 > 0:
        日志.输出控制台(f"GIS中更改以CAD为准地类色块的要素 中 存在部分 CAD色块 才拥有的地类")
    else:
        日志.输出控制台(f"GIS中更改以CAD为准地类色块的要素 中 无 CAD色块 才拥有的地类")
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        用地创建_生成用地规划图(
            ...
            GIS中更改以CAD为准地类色块的要素名称="YD_CAD色块以外建设用地修改",
            SQL_CAD地块中空隙的地类="'00'",
            SQL_以CAD为准的地类="地类编号 LIKE '07%' OR 地类编号 LIKE '08%' OR 地类编号 LIKE '09%' OR 地类编号 LIKE '10%' OR 地类编号 LIKE '11%'  OR 地类编号 LIKE '12%'  OR 地类编号 LIKE '13%'  OR 地类编号 LIKE '14%'  OR 地类编号 LIKE '15%'  OR 地类编号 LIKE '16%'  OR ( 地类编号 LIKE '17%' AND 地类编号 NOT LIKE '1704%' AND 地类编号 NOT LIKE '1705%' )  OR 地类编号 LIKE '2301%'",
            范围要素名称="JX_规划范围线",
            输出要素名称="DIST_用地规划图",
            是否拓扑检查=False,
            是否范围检查=False,

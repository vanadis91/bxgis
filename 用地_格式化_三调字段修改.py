...
def main(工作空间=r"C:\Users\common\project\J江东区临江控规\临江控规_数据库.gdb", 输入要素名称="CZ_三调"):  # 用地_格式化_三调字段修改
    with bxarcpy.类.环境.环境管理器(临时工作空间=工作空间, 工作空间=工作空间):
        ...
        for x in 替换字典:
            try:
                输入要素.字段修改(字段名称=x["字段名称"], 修改后字段名称=x["修改后名称"], 修改后字段别称="", 字段类型=x["类型"], 字段长度=x["长度"], 清除字段别称=True)
        ...
        调试.输出调试(f"筛选后要素是：" + str(筛选后要素))
        bxarcpy.类.要素类.要素读取_通过名称("CZ_三调筛选_扣除地类系数").要素删除()
        筛选后要素.要素创建_通过复制(r"CZ_三调筛选_扣除地类系数")

        SQL语句 = r"种植属性名称 = '工程恢复' Or 种植属性名称 = '即可恢复'"
        筛选后要素 = 输入要素.要素创建_通过筛选(SQL语句=SQL语句)
        筛选后要素.字段删除(保留字段名称列表=["种植属性名称"])
        调试.输出调试(f"筛选后要素是：" + str(筛选后要素))
        bxarcpy.类.要素类.要素读取_通过名称("CZ_三调筛选_种植属性名称").要素删除()
        筛选后要素.要素创建_通过复制(r"CZ_三调筛选_种植属性名称")

        SQL语句 = r"城镇村属性码 <> ' '"
        筛选后要素 = 输入要素.要素创建_通过筛选(SQL语句=SQL语句)
        筛选后要素.字段删除(保留字段名称列表=["城镇村属性码"])
        调试.输出调试(f"筛选后要素是：" + str(筛选后要素))
        bxarcpy.类.要素类.要素读取_通过名称("CZ_三调筛选_城镇村属性码").要素删除()
        筛选后要素.要素创建_通过复制(r"CZ_三调筛选_城镇村属性码")

        # SQL语句 = r"城镇村属性码 <> ' '"
        # 筛选后要素 = 输出要素.要素创建_通过筛选(SQL语句=SQL语句)
        筛选后要素 = 输入要素.要素创建_通过复制()
        筛选后要素.字段删除(保留字段名称列表=["坐落单位名称"])
        筛选后要素 = 筛选后要素.要素创建_通过融合(融合字段列表=["坐落单位名称"])
        调试.输出调试(f"筛选后要素是：" + str(筛选后要素))
        bxarcpy.类.要素类.要素读取_通过名称("CZ_三调筛选_坐落单位名称").要素删除()
        筛选后要素.要素创建_通过复制(r"CZ_三调筛选_坐落单位名称")

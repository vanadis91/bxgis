        ...
        with bxarcpy.游标类.游标创建_通过名称("更新", 融合后要素.名称, ["_形状", "是否切分", "_ID", "地类编号"]) as 游标:
            for x in 游标:
                ...
                if 图形.孔洞数量 >= 切分阈值["孔洞数量"] and 图形.折点数量 >= 切分阈值["折点数量"] and 图形.面积 >= 切分阈值["面积"] and x["地类编号"] in 切分阈值["地类编号列表"]:
                    x["是否切分"] = "1"
                    游标.行更新(x)
                    日志.输出控制台(f"ID：{str(x['_ID'])}，孔洞数量：{str(图形.孔洞数量)}, 折点数量：{str(图形.折点数量)}，面积：{str(图形.面积)}，地类：{x['地类编号']}")
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        转换_导出到CAD("DIST_用地规划图", 范围要素名称=None, 需融合地类编号列表=None, 切分阈值={"折点数量": 10, "孔洞数量": 2, "面积": 1000, "地类编号列表": ["1207"]}, 是否去孔=True, 输出CAD路径=r"C:\Users\beixiao\Desktop\01.dwg")

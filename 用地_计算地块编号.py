...
def 计算地块编号(输入要素名称, 输出要素名称="in_memory\\AA_计算地块编号"):
    ...
    公路要素 = 公路要素.要素创建_通过融合(["地类编号", "所属街坊", "所属街区", "用地构成"])
    公路要素.字段添加("地块编号")
    ...
    道路要素 = 道路要素.要素创建_通过融合(["地类编号", "所属街坊", "所属街区", "用地构成"], 是否单部件=False)
    道路要素.字段添加("地块编号")
    ...
    非建设用地要素 = 非建设用地要素.要素创建_通过融合(["地类编号", "用地构成"])
    非建设用地要素.字段添加("地块编号")
    需操作的字段名称列表 = ["地块编号"]
    非建设用地编号 = 0
    with bxarcpy.游标类.游标创建_通过名称("更新", 非建设用地要素.名称, 需操作的字段名称列表) as 游标:
        for x in 游标:
            非建设用地编号 += 1
            x["地块编号"] = "QT12" + "-" + str(非建设用地编号).zfill(2)
...
if __name__ == "__main__":
    ...
    with bxarcpy.环境.环境管理器(工作空间):
        计算地块编号(输入要素名称="DIST_用地规划图", 输出要素名称="DIST_用地规划图1")

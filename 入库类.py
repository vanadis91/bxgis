...
def 入库_单元(工作空间=r"C:\Users\common\project\J江东区临江控规\临江控规_数据库.gdb", 规划范围要素名称="JX_规划范围线", 单元编号="QT12", 单元名称="临江单元", 批复时间="", 批复文号="", 编制单位="浙江大学建筑设计研究院有限公司", 单元功能="", 人口规模=3.15):
...
def 入库_街区街坊(工作空间=r"C:\Users\common\project\J江东区临江控规\临江控规_数据库.gdb", 区域要素名称="JX_街区范围线", 单元名称="临江单元", 层级="街区", 输出要素名称="XG_JQJF_JQ"):
    with bxarcpy.环境.环境管理器(临时工作空间=工作空间, 工作空间=工作空间):
        # To allow overwriting outputs change overwriteOutput option to True.
        bxarcpy.配置.是否覆盖输出要素 = True
        print(f"当前层级是：{层级}")
...
def 入库_生成土地码(工作空间=r"C:\Users\common\project\J江东区临江控规\临江控规_数据库.gdb", 地块要素名称="XG_GHDK"):
    with bxarcpy.环境.环境管理器(临时工作空间=工作空间, 工作空间=工作空间):
        bxarcpy.配置.是否覆盖输出要素 = True
        地块要素 = bxarcpy.要素类.要素读取_通过名称(地块要素名称)
        地块要素 = 地块要素.要素创建_通过复制()

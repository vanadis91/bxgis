...
def 用地_计算调整类型(用地调整要素名称, 输出要素名称="in_memory\\AA_计算调整类型"):
    ...
    输入要素 = bxarcpy.要素类.要素读取_通过名称(用地调整要素名称).要素创建_通过复制()
    输入要素.字段添加("调整类型").字段添加("调整类型1")
    with bxarcpy.游标类.游标创建_通过名称("更新", 输入要素.名称, ["地类编号_现状", "地类编号_规划", "地类差异", "调整类型", "调整类型1", "所属三区三线", "开发动态", "Shape_Area", "容积率"]) as 游标:
